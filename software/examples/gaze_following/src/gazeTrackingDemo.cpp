...
public:
    ...
    void getRobotState(bot_core::robot_state_t &robot_state_msg, int64_t utime_in, Eigen::VectorXd q,
    ...
    void robotStateHandler(const lcm::ReceiveBuffer *rbuf, const std::string &channel,
                           const bot_core::robot_state_t *msg);

    void gazeGoalHandler(const lcm::ReceiveBuffer *rbuf, const std::string &channel, const bot_core::vector_3d_t *msg);

    void aprilTagTransformHandler(const lcm::ReceiveBuffer *rbuf, const std::string &channel,
...
/////////////////////////////////////////////////
void App::getRobotState(bot_core::robot_state_t &robot_state_msg, int64_t utime_in, Eigen::VectorXd q,
                        std::vector <std::string> jointNames) {
...
void App::solveGazeProblem() {
    ...
    Eigen::VectorXd q_star(robotStateToDrakePosition(rstate_, dofMap_, model_.num_positions));
    ...
    if (mode_ ==
        TrackingControlMode::DESIRED_HEAD_ORIENTATION) { // publish utorso-to-head as orientation, not properly tracking but works with different orientations
        ...
        lcm_->publish("DESIRED_HEAD_ORIENTATION", &utorso_to_head_frame_pose_msg);// temp
               ...
               TrackingControlMode::JOINT_POSITION_GOAL) { // publish neck pitch and yaw joints as orientation. this works ok when robot is facing 1,0,0,0
        std::vector <std::string> jointNames;
