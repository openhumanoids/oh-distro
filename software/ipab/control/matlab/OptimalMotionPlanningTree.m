...
%     xGoal         
  ...
  methods
    ...
    function qPath = rebuildTraj(obj, x_start, x_goal)      
...
%           drawTreePoints(pt(1:3), 'text', 'Checked Vertex', 'colour', [1 1 1])
...
%           drawTreePoints(edgePath, 'text', 'EdgePath', 'colour', [1 1 0]);
        end
        qPath = [qPath edgePath];
      end
      qPath = [qPath x_goal];
%       drawTreePoints([], 'text', 'Checked Vertex', 'colour', [1 1 1])
%       drawTreePoints([], 'text', 'EdgePath', 'colour', [1 1 0]);
%       drawTreePoints(qPath, 'text', 'qPath', 'colour', [1 0 1]);
%       drawTreePoints(qPath, 'text', 'qPath', 'colour', [1 0 1]);
    end
      ...
      if valid
        ...
        for q = 1:size(QNear,1)
          ...
          if cost + .1 * obj.C(obj.traj(1)) < obj.C(QNear(q, 1)) && obj.checkEdge(obj.getVertex(idNew), obj.getVertex(QNear(q, 1)))
            ...
            if status == Status.GOAL_REACHED && any(obj.traj == q)
...
%         while status == Status.ADVANCED;
          [obj, idNearest] = obj.addVertex(qNew, idNearest);
%           [qNew, idNearest, status] = obj.extend(idNearest, qRand);
%         end
      end
    end
    
    function [qNew, idNearest, status] = extend(obj, idNearest, qRand)
      qNew = obj.steer(idNearest, qRand);
...
%           drawTreePoints(pt(1:3), 'text', 'Checked Vertex', 'colour', [1 1 1])
          constraints = obj.generateEndEffectorConstraints(pt(1:7));
          [qNew, valid, infeasibleConstraints] = cSpaceTree.solveIK(pt(8:end), qNew, constraints);
          if ~valid %if kinematics fails return a kinematic fail
            info = Info(Info.FAIL_KINEMATIC_CONSTRAINT, qNew, infeasibleConstraints);
...
%       drawTreePoints([], 'text', 'Checked Vertex', 'colour', [1 1 1])
    ...
    function qRand = goalBias(obj, status, xGoal, iter, spacing)
      ...
      if status ~= Status.GOAL_REACHED && any(goalIdx)
%         drawTreePoints(xGoal(:, goalIdx), 'text', 'BiasVertex', 'pointsize', 0.02, 'colour', [1 1 0]);
        qRand = xGoal(:, goalIdx);
      else
    ...
    function [qNew, idNearest, valid] = newPoint(obj, qRand, status, iter)
      ...
      qNew = [];
      idNearest = [];
      if status == Status.GOAL_REACHED && mod(iter, 4) ~= 1
        subSet = obj.traj;
      else
        subSet = 1:obj.n;
      end
...
%       drawTreePoints(pt, 'colour', [1 0 0], 'text', 'Checked Vertex');
    ...
    function [QNear, q] = near(obj, q)
      qPose = q(1:7);
      d = 7; %for 3D space
      r = min([obj.gamma*(log(obj.n)/obj.n)^(1/d), obj.eta]); %obj.steerFactor]);
    ...
    function [obj, cost] = getTrajToGoal(obj, goalId)
      ...
      obj.traj(end+1) = 1;  
      cost = obj.C(goalId);
      drawTreePoints(obj.traj, 'lines', true, 'tree', obj, 'text', 'Optimal Trajectory');
%       csTree = obj.trees{obj.cspace_idx};
%       pos = obj.trees{obj.tspace_idx}.trees{1}.getVertex(obj.traj);
%       quat = obj.trees{obj.tspace_idx}.trees{2}.getVertex(obj.traj);
%       visualizeCollisionMesh(csTree.rbm, obj.end_effector_id, csTree.getVertex(1), pos, quat, obj.end_effector_pt);
    end
    ...
    function obj = shortcut(obj)
      ...
      while i < nIter
        ...
        b = obj.traj(bIdx);
        i = i+1;
%         drawTreePoints([a,b], 'tree', obj, 'lines', true, 'colour', [1 0 0]);
        cost = obj.cost(a,b);
