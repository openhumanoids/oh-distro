...
class rgb_tool{
  ...
  private:
    ...
    void kinectHandler(const lcm::ReceiveBuffer* rbuf, const std::string& channel, const  openni::frame_msg_t* msg); 
...
rgb_tool::rgb_tool(lcm::LCM* &lcm_): lcm_(lcm_){
  ...
  lcm_->subscribe( "KINECT_DATA",&rgb_tool::kinectHandler,this);

}

void rgb_tool::kinectHandler(const lcm::ReceiveBuffer* rbuf, const std::string& channel, const  openni::frame_msg_t* msg){
