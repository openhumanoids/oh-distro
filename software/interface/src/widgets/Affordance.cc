...
ExampleWindow::ExampleWindow(boost::shared_ptr<lcm::LCM> _lcm, boost::shared_ptr<otdf::ModelInterface> _otdfObject) :
			m_Button_Quit("Quit"), m_DrawingArea(_otdfObject, _lcm)
{
this->otdfObject = _otdfObject;
	...
	m_DrawingArea.set_size_request(400, 500);
	m_HBox.pack_start(m_DrawingArea, Gtk::PACK_EXPAND_WIDGET);
	...
	for(std::map<string,double>::iterator ite = params.begin(); ite != params.end(); ++ite) {
	  ...
	  m_HBox_Pairing->pack_start(*Gtk::manage(new TwoStateScale(dofName, _lcm)), Gtk::PACK_EXPAND_WIDGET);
