...
RobotStateListener::RobotStateListener(boost::shared_ptr<lcm::LCM> &lcm, MainDrawingArea * parentPane):
    		_urdf_parsed(false),
    		_lcm(lcm),
    		_viewer(parentPane)
...

{
	...
	_urdf_subscription = _lcm->subscribe("ROBOT_MODEL", &gui::RobotStateListener::handleRobotUrdfMsg,this);
	_urdf_subscription_on = true;
	if (_urdf_subscription != 0) {
		cout << "URDF Subscription not null" << endl;
	} else {
		cout << "URDF Subscription is NULL" << endl;
	}
	//Subscribes to MEAS_JOINT_ANGLES
	//lcm->subscribe("MEAS_JOINT_ANGLES", &RobotStateListener::handleJointAnglesMsg, this); //&this ?
	_lcm->subscribe("EST_ROBOT_STATE", &gui::RobotStateListener::handleRobotStateMsg, this); //&this ?
}
		...
		if(it->second->visual)
		{
			...
			if(transform_it!=cartpos_out.end())// fk cart pos exists
			{
				...
				if (type == SPHERE){
					...
					//_viewer->center = it->second->visual->origin.position;

					_viewer->center->x = it->second->visual->origin.position.x;
					_viewer->center->y = it->second->visual->origin.position.y;
					_viewer->center->z = it->second->visual->origin.position.z;

					_viewer->eye->x = it->second->visual->origin.position.x;
					_viewer->eye->y = it->second->visual->origin.position.y;
					_viewer->eye->z = it->second->visual->origin.position.z;
				...
				//cout << "\t .x  : " << state.tf.tranoslation.x << endl;
...
{
	cout << "DEBUG: Number of links = " << _link_tfs.size() << endl;
