...
void App::batchHandler(const lcm::ReceiveBuffer* rbuf, const std::string& channel, const  drc::atlas_raw_imu_batch_t* msg){
  ...
  drc::atlas_raw_imu_batch_t msg_copy = drc::atlas_raw_imu_batch_t(*msg);
  ...
  for (int i=0 ; i < batch.packets.size() ; i++ ){ //oldest to newest
    drc::atlas_raw_imu_t raw_msg = imu_data_.convertToLCMPacket( batch.packets[i] );
  ...
  for (int i=0 ; i < batch.packets.size() ; i++ ){ //oldest to newest
    drc::atlas_raw_imu_t imu_filtered_msg = imu_data_.convertToLCMPacket( batch.packets[i]);
  ...
  if (imu_data_.getNumberBatchSinceReset()  > 50){
    drc::atlas_raw_imu_batch_t batch_filtered_msg = imu_data_.convertToLCMBatch(batch);
