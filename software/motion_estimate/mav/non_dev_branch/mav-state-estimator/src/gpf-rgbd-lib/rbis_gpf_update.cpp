...
RBISRgbdGPFMeasurement::RBISRgbdGPFMeasurement(RgbdGPF *gpf_, kinect_frame_msg_t * laser_msg_, int64_t utime,
...
RgbdGPFHandler::RgbdGPFHandler(lcm_t * lcm_, BotParam * param, BotFrames * frames) :
    lcm(lcm_), SensorHandler(param, "rgbd_gpf")
{
  gpf = new RgbdGPF(lcm, param, frames);
  char * chan = bot_param_get_str_or_fail(param, "state_estimator.rgbd_gpf.rgbd_channel");
  rgbd_channel = chan;
  free(chan);

}

RBISUpdateInterface * RgbdGPFHandler::processMessage(const kinect_frame_msg_t * msg)
{
  if (counter++ % downsample_factor != 0)
    return NULL;

  // mfallon utime changed to timestamp
  return new RBISRgbdGPFMeasurement(gpf, kinect_frame_msg_t_copy(msg), msg->timestamp - utime_delay, lcm, channel); 
}
