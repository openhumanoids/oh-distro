...
public:
  app_t(int argc, char ** argv)
  {
    ...
    laser_queue = new deque<bot_core_planar_lidar_t *>();
    ...
    bot_core_planar_lidar_t_subscribe(lcm, laser_handler->laser_channel.c_str(), laser_message_handler, (void *) this);
  ...
  deque<bot_core_planar_lidar_t *> * laser_queue;
    ...
    void * user)
{
  ...
  bot_core_planar_lidar_t * msg_copy = bot_core_planar_lidar_t_copy(msg);
  g_mutex_lock(app->lcm_data_mutex);
  if (!app->noDrop) {
    //clear the old messages
    while (!app->laser_queue->empty()) {
      bot_core_planar_lidar_t * msg = app->laser_queue->front();
      bot_core_planar_lidar_t_destroy(msg);
...
static void * processing_func(void * user)
{
  ...
  while (1) {
    ...
    bot_core_planar_lidar_t * laser_msg = app->laser_queue->front();
    app->laser_queue->pop_front();
