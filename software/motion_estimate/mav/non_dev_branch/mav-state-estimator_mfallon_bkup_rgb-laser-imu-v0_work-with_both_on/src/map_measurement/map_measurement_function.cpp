    ...
    for (int j = 0; j < psi_dim; j++) {
      ...
      for (int k = 0; k < x_dim; k++) {
        for (int l = 0; l < y_dim; l++) {
          ...
          const bot_core_planar_lidar_t * laser_scan = laser_sim.simulate(&bot_trans);

          laser_gpf.getMeasurement(state, prior_cov, laser_scan, z_effective, R_effective);
          ...
          if (visualize) {
            ...
            bot_core_planar_lidar_t_publish(lcm, "LASER", laser_scan);
