...
def keypress(c):
  ...
  if c in g_key_map:
    pub = None
    msg = sandia_simple_grasp_t()
    msg.name=g_key_map[c][1]
    msg.closed_amount=g_key_map[c][2]
    if g_key_map[c][0] == 'L':
      lc.publish("SANDIA_LEFT_SIMPLE_GRASP", msg.encode())
    elif g_key_map[c][0] == 'R':
      lc.publish("SANDIA_RIGHT_SIMPLE_GRASP", msg.encode())
    else:
      return # wtf
...
if __name__ == '__main__':
  ...
  draw_string(screen, font, 10, 70, "Input:")
  draw_string(screen, font, 10, 90, "LEFT   Shape   Right")
  draw_string(screen, font, 10, 110, "1--5 Cylinder  6--9")
