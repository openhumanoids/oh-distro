function test_mb()
    ...
    while ( true )
        ...
        if cam_getResetAndClear(ptr)
            ...
            R = eye(length(x)) * 0.05^2;  % system noise
            P = R;
        else
        ...
        figure(1);
        clf;
        ph = DrawAxis(GetHomoTransform(prevX(1:6)), 0.1, 'b', 'b', 'k');
        hold on;
        ch = DrawAxis(GetHomoTransform(x(1:6)), 0.1, 'b', 'b', 'r');
        z = reshape(zk, 3, [])';
        ...
        for i = 2:size(sigmaX,2)
            DrawAxis(GetHomoTransform(sigmaX(1:6,i)), 0.03, 'b', 'b', 'k');
            DrawAxis(GetHomoTransform(sigmaNewX(1:6,i)), 0.03, 'b', 'b', 'r');
        end
