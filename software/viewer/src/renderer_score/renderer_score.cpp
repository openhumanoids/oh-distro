//this is the renderer_status_code
//used just to add the new renderer structure to the repo
...
using namespace std;
#define RENDERER_NAME "System_Score"
...
on_ground_driving_status(const lcm_recv_buf_t *rbuf,
    ...
    RendererSystemScore *self = (RendererSystemScore*) user;
...
on_pose_body(const lcm_recv_buf_t * buf, const char *channel, const bot_core_pose_t *msg, void *user_data){
    RendererSystemScore *self = (RendererSystemScore*) user_data;
    double rpy_in[3];
...
static void _draw(BotViewer *viewer, BotRenderer *r){
    RendererSystemScore *self = (RendererSystemScore*)r;
    ...
    sprintf(line1, "The SCOREBOARD backend is not implemented yet");
    sprintf(line2, "This is just the render");
    sprintf(line3, "Check back later");
    sprintf(line4, "Turn off this render in the meantime");
...
static void _destroy(BotRenderer *r){
    ...
    RendererSystemScore *self = (RendererSystemScore*)r->user;
    free(self);
}

BotRenderer *renderer_score_new(BotViewer *viewer, int render_priority, lcm_t *lcm){
        RendererSystemScore *self = (RendererSystemScore*)calloc(1, sizeof(RendererSystemScore));
        
        self->lcm = lcm;
        self->viewer = viewer;
        BotRenderer *r = &self->renderer;
        self->renderer.name = "Scoreboard";
...
void score_add_renderer_to_viewer(BotViewer *viewer, int render_priority, lcm_t *lcm){
  bot_viewer_add_renderer(viewer, renderer_score_new(viewer,
