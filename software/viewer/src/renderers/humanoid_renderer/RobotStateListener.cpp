...
namespace fk 
{
    ...
    _collision_detector()
  {
    ...
    //lcm->subscribe("MEAS_JOINT_ANGLES", &fk::RobotStateListener::handleJointAnglesMsg, this); 
    lcm->subscribe("EST_ROBOT_STATE", &fk::RobotStateListener::handleRobotStateMsg, this); 
      ...
      if(it->second->visual)
      {
        ...
        if(transform_it!=cartpos_out.end())// fk cart pos exists
        {
          ...
          if  (type == MESH)
          {
            ...
            _collision_object_map.find(it->first)->second.set_transform(p,q); 
        ...
        if(it->second->visual)
        {
          ...
          if  (type == MESH)
          {
           ...
           // populate data structures
            ...
            _collision_object_map.insert(make_pair(it->first, collision_object));
             
            // add a collision object to the collision detector class
            _collision_detector.add_collision_object(&_collision_object_map.find(it->first)->second);
