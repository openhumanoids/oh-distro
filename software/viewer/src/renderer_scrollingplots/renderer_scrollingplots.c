...
static void scrolling_plots_draw (BotViewer *viewer, BotRenderer *renderer)
{
    ...
    if (bot_gtk_param_widget_get_bool (self->pw, PARAM_NAME_RENDER_VO)) {
        bot_gl_scrollplot2d_gl_render_at_window_pos (self->vo_plot, 
                x, y, plot_width, plot_height);
        y += plot_height;
    }
...
void scrollingplots_add_renderer_to_viewer(BotViewer* viewer, int priority, lcm_t* lcm)
{
  ...
  bot_gtk_param_widget_add_booleans (self->pw, 0,
      ...
      PARAM_NAME_RENDER_PITCHROLL_HEAD, 1,
      PARAM_NAME_RENDER_HEIGHT, 1,
      PARAM_NAME_RENDER_VO, 1,
  ...
  bot_gl_scrollplot2d_add_plot    (self->height_plot, "height", MAX_POINTS);
  bot_gl_scrollplot2d_set_color   (self->height_plot, "height", 0, 0, 1, 1);
  ...
  self->vo_plot = bot_gl_scrollplot2d_new ();
  bot_gl_scrollplot2d_set_title        (self->vo_plot, "VO Status");
  bot_gl_scrollplot2d_set_text_color   (self->vo_plot, 0.7, 0.7, 0.7, 1);
  bot_gl_scrollplot2d_set_bgcolor      (self->vo_plot, 0.1, 0.1, 0.1, 0.7);
  bot_gl_scrollplot2d_set_border_color (self->vo_plot, 1, 1, 1, 0.7);
  bot_gl_scrollplot2d_set_ylim    (self->vo_plot, -1, 4); // was 0 to 5 until changed fovision output
  bot_gl_scrollplot2d_add_plot    (self->vo_plot, "vo", MAX_POINTS);
