...
// ===================================================================
  inline static double get_shortest_distance_between_keyframes_and_markers (void *user,Eigen::Vector3f &from,Eigen::Vector3f &to)
  {
    ...
    if((self->robotPlanListener->_gl_left_hand)&&(self->robotPlanListener->_gl_right_hand)) // exists
      {
      ...
      &&(!self->robotPlanListener->_gl_right_hand->is_bodypose_adjustment_enabled())) // knot points other than end points exist
     {
        ...
        for(uint i = 1; i < self->robotPlanListener->_gl_robot_keyframe_list.size()-1; i++) // ignore current state and end state, only intermediate stuff
         //for(uint i = 0; i < self->robotPlanListener->_gl_robot_keyframe_list.size(); i++) 
        { 
...
// ===================================================================
  ...
  inline static void publish_traj_opt_constraint(void *user,string &channel, int index)
  {
    ...
    KDL::Frame T_worldframe_palm_l,T_worldframe_palm_r;         
      ...
      if(self->is_left_in_motion) {
       ...
       KDL::Frame DragRotation=KDL::Frame::Identity();
        ...
        if((*self->marker_selection)=="markers::base_x"){
          ...
          currentAngle = atan2(self->prev_ray_hit_drag[2]-T_world_hand.p[2],self->prev_ray_hit_drag[1]-T_world_hand.p[1]);
          angleTo = atan2(self->ray_hit_drag[2]-T_world_hand.p[2],self->ray_hit_drag[1]-T_world_hand.p[1]);
          ...
          currentAngle = atan2(self->prev_ray_hit_drag[0]-T_world_hand.p[0],self->prev_ray_hit_drag[2]-T_world_hand.p[2]);
          angleTo = atan2(self->ray_hit_drag[0]-T_world_hand.p[0],self->ray_hit_drag[2]-T_world_hand.p[2]);
          dtheta = gain*shortest_angular_distance(currentAngle,angleTo);
          //dtheta =  atan2(sin(angleTo - currentAngle), cos(angleTo - currentAngle));
          KDL::Vector axis;
          ...
          currentAngle = atan2(self->prev_ray_hit_drag[1]-T_world_hand.p[1],self->prev_ray_hit_drag[0]-T_world_hand.p[0]);
          angleTo = atan2(self->ray_hit_drag[1]-T_world_hand.p[1],self->ray_hit_drag[0]-T_world_hand.p[0]);
          dtheta = gain*shortest_angular_distance(currentAngle,angleTo);
          KDL::Vector axis;
          axis[0] = 0; axis[1] = 0; axis[2]=1;
          DragRotation.M = KDL::Rotation::Rot(axis,dtheta);
        }
        T_world_hand.M  = DragRotation.M*T_world_hand.M;  
        
        std::map<std::string, double> jointpos_in;
        
        if(self->is_left_in_motion) {
          jointpos_in = self->robotPlanListener->_gl_left_hand->_current_jointpos;
          self->robotPlanListener->_gl_left_hand->set_state(T_world_hand,jointpos_in); 
        }
        else {
          jointpos_in = self->robotPlanListener->_gl_right_hand->_current_jointpos;
          self->robotPlanListener->_gl_right_hand->set_state(T_world_hand,jointpos_in); 
        }
