...
typedef struct _RendererEndEffectorGoal {
	...
	//std::string channel;	
  boost::shared_ptr<renderer_ee_goal::RobotStateListener> robotStateListener;
...
BotRenderer *renderer_end_effector_goal_new (BotViewer *viewer, int render_priority, lcm_t *lcm)
{
  ...
  self->_lcm = boost::shared_ptr<lcm::LCM>(new lcm::LCM(lcm));
	self->robotStateListener = boost::shared_ptr<renderer_ee_goal::RobotStateListener>(new renderer_ee_goal::RobotStateListener(self->_lcm,viewer));
