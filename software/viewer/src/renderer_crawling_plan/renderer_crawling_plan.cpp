...
#define RENDERER_NAME "Crawling Plan Display"
...
using namespace renderer_crawling_plan_gui_utils;
...
draw_state(BotViewer *viewer, BotRenderer *super, uint i, float rgb[]){
  ...
  if((self->use_colormap)&&(self->displayed_plan_index==-1)&&(!self->robotPlanListener->_is_keyframe_plan)) {
...
_renderer_draw (BotViewer *viewer, BotRenderer *super)
{
  ...
  // if any plan execution/approval dock's exist, they will be destroyed.
   ...
   // Show keyframes
  if((self->show_keyframes)&&(self->robotPlanListener->_is_keyframe_plan)&&(self->robotPlanListener->_gl_robot_keyframe_list.size()>0))
  {
...
static int mouse_press (BotViewer *viewer, BotEventHandler *ehandler, const double ray_start[3], const double ray_dir[3], const GdkEventButton *event)
{
 ...
 // fprintf(stderr, "RobotPlanRenderer Ehandler Activated\n");
  ...
  if((self->robotPlanListener->_is_keyframe_plan)&&(((*self->selection)  != " ") || ((*self->marker_selection)  != " "))&&(event->button==1) &&(event->type==GDK_2BUTTON_PRESS))
  {
...
void 
setup_renderer_crawling_plan(BotViewer *viewer, int render_priority, lcm_t *lcm, int operation_mode)
{
