...
//    _parent_renderer(parent_renderer)
    ...
    _lcm(lcm),_viewer(viewer)
  {
    ...
    if (operation_mode==0){ // typical mode
      lcm->subscribe("FOOTSTEP_PLAN_RESPONSE", &renderer_sticky_feet::FootStepPlanListener::handleFootStepPlanMsg, this);
...
void FootStepPlanListener::handleFootStepPlanMsg(const lcm::ReceiveBuffer* rbuf,
						 ...
						 const drc::footstep_plan_t* msg)						 
  {
    ...
    bool is_new_plan = true;
 
    if(_waiting_for_new_plan && !is_new_plan) {
    ...
    for (uint i = 0; i <(uint)num_steps; i++){
      drc::footstep_t goal_msg  = msg->footsteps[i]; 
