...
#define PARAM_LOOK_AT "Look At Z"
...
grid_draw (BotViewer *viewer, BotRenderer *renderer)
{
    ...
    look[2] = bot_gtk_param_widget_get_double(self->pw, PARAM_LOOK_AT);
...
//    gtk_label_set_text (GTK_LABEL (self->label), txt);
    ...
    double grid_oz = look[2]; //always want the grid at 0
...
static BotRenderer *renderer_drcgrid_new (BotViewer *viewer)
{
...
//    gtk_widget_show (self->label);
    ...
    bot_gtk_param_widget_add_double(self->pw, 
            PARAM_LOOK_AT, BOT_GTK_PARAM_WIDGET_SPINBOX, 0, 10, 0.1, 1);
...
{
    bot_viewer_add_renderer(viewer, renderer_drcgrid_new(viewer), priority);
}
