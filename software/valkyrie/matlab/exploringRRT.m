function [xtraj, info, v, simVars] = exploringRRT(options, rng_seed)
% TIMEOUT 600
if nargin < 1 || isempty(options), options = struct(); end
if nargin < 2
    s=rng;
    save(sprintf('rndSeed'), 's');
else
    rng(rng_seed);
end
...
qpath = extractPath(T_smooth, path_ids_A);
...
simVars.rrtTime = rrt_time;
simVars.smoothingTime = smoothing_time;
simVars.TA = TA;
simVars.TB = TB;
