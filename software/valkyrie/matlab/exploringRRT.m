...
if ~isfield(options,'scene'), options.scene = 'scene3'; end;
...
S = Scenes.getFP(options.model); %load([getDrakePath(), '/examples/Atlas/data/atlas_fp.mat']);
q_nom = S.xstar(1:nq);
...
startPoseConstraints = Scenes.setInitialPoseConstraints(options, r);
...
endPoseConstraints = Scenes.setEndPoseConstraints(options, r);
...
if strcmp(options.scene, 'debris')
        ...
        %l_hand_initial_constraint = Scenes.setStartPoseConstraints(options, r);
...
ikoptions = ikoptions.setMajorOptimalityTolerance(1e-3);
TA.trees{TA.cspace_idx}.ikoptions = ikoptions;
nominalPoseConstraints = Scenes.setNominalPoseConstraints(options, r);
[q_start, info, infeasible_constraint] = inverseKin(r, ik_seed_pose, ik_nominal_pose, nominalPoseConstraints{:}, ikoptions);
...
nominalPoseConstraints{7} = baseConstraint;


active_constraints = [nominalPoseConstraints, {l_hand_final_pos_constraint,l_hand_final_quat_constraint}];
...
active_constraints = [Scenes.setStartPoseConstraints(options, r), {position_constraint_7,quat_constraint_8}];%[nominalPoseConstraints, {l_hand_initial_constraint}]; %{position_constraint_7,quat_constraint_8}];
