<xsl:stylesheet version="2.0" 
  ...
  <xsl:template match="model">
    <object name="{@name}">
      ...
      <xsl:value-of select="$newline" />
      <xsl:comment>X,Y,Z,Roll,Pitch,Yaw params must exist for all otdf objects</xsl:comment> 
      <xsl:value-of select="$newline" />
      <xsl:comment>World frame origin of the object</xsl:comment>
      <xsl:value-of select="$newline" />
      ...
      <xsl:value-of select="$newline" />
      <xsl:value-of select="$newline" />
      <xsl:comment>Roll,Pitch,Yaw are about Body Frame</xsl:comment>
      <xsl:value-of select="$newline" />
  ...
  <xsl:template match="link">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise> <!--Create fixed joint to world-->
        <joint name="{concat($link-name,'_joint')}" type="fixed">
          <xsl:call-template name="origin">
            <xsl:with-param name="T" select="$o_X_c"/>
          </xsl:call-template>
          <parent link="base_link" type="link"/>
          <child link="{$link-name}" type="link"/>
        </joint>
      </xsl:otherwise>
    </xsl:choose>
    <!--<xsl:value-of select="$newline" />-->
