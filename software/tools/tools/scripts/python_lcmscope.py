...
from drc.foot_contact_estimate_t import foot_contact_estimate_t
########################################################################################
...
def plot_data():
  global last_utime
  front_block =100000 # offset into the future (to ensure no recent data not viewed)
  if ( len(pos.utimes) >1):
    plt.figure(1)
    ############################################################
    ax1.cla()
    #print "posrate %f %f"%( posrate.utimes.size, posrate.v.size)
    ax1.plot(posrate.utimes[2:], np.transpose(posrate.v[2:,0]), 'r', linewidth=1,label="xdot")
    ax1.plot(posrate.utimes[2:], np.transpose(posrate.v[2:,1]), 'g', linewidth=1,label="ydot")
    ax1.plot(posrate.utimes[2:], np.transpose(posrate.v[2:,2]), 'b', linewidth=1,label="zdot")
    #
    ax1.plot(posrate_est.utimes[2:], np.transpose(posrate_est.v[2:,0]), 'r.', linewidth=1,label="xdot est")
    ax1.plot(posrate_est.utimes[2:], np.transpose(posrate_est.v[2:,1]), 'g.', linewidth=1,label="ydot est")
    ax1.plot(posrate_est.utimes[2:], np.transpose(posrate_est.v[2:,2]), 'b.', linewidth=1,label="zdot est")
    ...
    ax1.set_xlim( float(last_utime - plot_window - first_utime)/1000000 ,  float(last_utime + front_block - first_utime)/1000000 )
    #ax1.set_ylim(-.5,.5)    
    ############################################################
    ax2.cla()
    ...
    #
    ax2.plot(angrate_est.utimes[2:], np.transpose(angrate_est.v[2:,0]), 'r.', linewidth=1,label="roll rate est")
    ax2.plot(angrate_est.utimes[2:], np.transpose(angrate_est.v[2:,1]), 'm.', linewidth=1,label="pitch rate est")
    ax2.plot(angrate_est.utimes[2:], np.transpose(angrate_est.v[2:,2]), 'y.', linewidth=1,label="yaw rate est")
    #
    ...
    ax2.set_xlim( float(last_utime - plot_window - first_utime)/1000000 ,  float(last_utime + front_block - first_utime)/1000000 )
    ############################################################
    ...
    ax4.set_xlim( float(last_utime - plot_window - first_utime)/1000000 ,  float(last_utime + front_block - first_utime)/1000000 )
  ...
  if (len(pos.utimes) >1):
    ...
    ax5.plot(pos.utimes[1:], np.transpose(pos.v[1:,0]), 'r', linewidth=1,label="x")
    ax5.plot(pos.utimes[1:], np.transpose(pos.v[1:,1]), 'g', linewidth=1,label="y")
    ax5.plot(pos.utimes[1:], np.transpose(pos.v[1:,2]), 'b', linewidth=1,label="z")
    #
    ax5.plot(pos_est.utimes[1:], np.transpose(pos_est.v[1:,0]), 'r.', linewidth=1,label="x est")
    ax5.plot(pos_est.utimes[1:], np.transpose(pos_est.v[1:,1]), 'g.', linewidth=1,label="y est")
    ax5.plot(pos_est.utimes[1:], np.transpose(pos_est.v[1:,2]), 'b.', linewidth=1,label="z est")
    ax5.legend();  ax5.set_xlabel('Time '+ str(last_utime));  ax5.set_ylabel('Local Pos [m]');  ax5.grid(True)
    ax5.legend(loc=2,prop={'size':10})
    ax5.set_xlim( float(last_utime - plot_window - first_utime)/1000000 ,  float(last_utime + front_block - first_utime)/1000000 )
    ############################################################
    ax6.cla()
    ...
    #
    ax6.plot(ang_est.utimes[1:], np.transpose(ang_est.v[1:,0]) *180/math.pi , 'r.', linewidth=1,label="roll est")
    ax6.plot(ang_est.utimes[1:], np.transpose(ang_est.v[1:,1]) *180/math.pi , 'm.', linewidth=1,label="pitch est")
    ax6.plot(ang_est.utimes[1:], np.transpose(ang_est.v[1:,2]) *180/math.pi , 'y.', linewidth=1,label="yaw est")
    ...
    ax6.set_xlim( float(last_utime - plot_window - first_utime)/1000000 ,  float(last_utime + front_block - first_utime)/1000000 )
    ############################################################
    ...
    ax7.set_xlim( float(last_utime - plot_window - first_utime)/1000000 ,  float(last_utime + front_block - first_utime)/1000000 )
    ...
    ax8.set_xlim( float(last_utime - plot_window - first_utime)/1000000 ,  float(last_utime + front_block - first_utime)/1000000 )
  ...
  if (len(utime_10hz_rate.utimes) >1):
    ...
    ax9.plot(foot_contact_est.utimes[1:],     np.transpose(foot_contact_est.v[1:,0]), 'g.-',  linewidth=1,label="Left Est")
    ax9.plot(foot_contact_est.utimes[1:],     np.transpose(foot_contact_est.v[1:,1]), 'm.-',  linewidth=1,label="Right Est")    
    
    ax9.legend(loc=2,prop={'size':10});   ax9.set_xlabel('Time '+ str(last_utime));   ax9.set_ylabel('Force [?? units]');   ax9.grid(True)
    ax9.set_xlim( float(last_utime - plot_window - first_utime)/1000000 ,  float(last_utime + front_block - first_utime)/1000000 )
    ax9.set_ylim( -250, 1250  )
    #########################################################################################################
    ax10.cla()
    ...
    ax10.set_xlim( float(last_utime - plot_window - first_utime)/1000000 ,  float(last_utime + front_block - first_utime)/1000000 )
    #########################################################################################################
    ax11.cla()
    ...
    ax11.set_ylim(0, 1);
    ax11.set_xlim( float(last_utime - plot_window - first_utime)/1000000 ,  float(last_utime + front_block - first_utime)/1000000 )
    ############################################################
    ax12.cla()
...
def on_true_robot_state(channel, data):
  ...
  if (m.utime < last_utime):
    print "out of order RS msg, resetting now %s | last %s"   %(m.utime,last_utime)
    reset_all()
  last_utime = m.utime
...
def on_est_robot_state(channel, data):
  m = robot_state_t.decode(data)
  pos_est.append(m.utime,  [m.origin_position.translation.x, m.origin_position.translation.y, m.origin_position.translation.z ] )
  #rpy_e = quat_to_euler([m.origin_position.rotation.w, m.origin_position.rotation.x, m.origin_position.rotation.y, m.origin_position.rotation.z] )
  #print "e: %f %f %f" %(rpy_e[0]*180.0/math.pi,rpy_e[1]*180.0/math.pi,rpy_e[2]*180.0/math.pi)
...
def on_foot_state(channel, data):
  m = foot_contact_estimate_t.decode(data)
  foot_contact_est.append(m.utime,  [1300*m.left_contact-200, 1300*m.right_contact-100] )
...
last_utime=0
first_utime=0
plot_window=1*1000000 #3sec
pos = SensorData(3); pos_est = SensorData(3);  
posrate = SensorData(3); posrate_est = SensorData(3);
ang = SensorData(3); ang_est = SensorData(3);  
angrate = SensorData(3); angrate_est = SensorData(3);
# sensed:
...
def lcm_thread():
  ...
  lc.subscribe("FOVIS_REL_ODOMETRY", on_relvo)

  
  # DRC msgs
  lc.subscribe("TRUE_ROBOT_STATE", on_true_robot_state)
  lc.subscribe("EST_ROBOT_STATE", on_est_robot_state)

  lc.subscribe("TORSO_IMU", on_imu)
  lc.subscribe("ROBOT_UTIME", on_utime)

  lc.subscribe("FOOT_CONTACT_ESTIMATE", on_foot_state)
  
  lc.subscribe("POSE_IMU_WORLD", on_pose_imu_world)
