...
#include "sticky_hand_lcm_utils.hpp"
using namespace std;
...
namespace visualization_utils
{
     ...
     drc::ee_goal_t get_eegoal_to_sticky_hand(StickyHandStruc &sticky_hand_struc, string robot_name,string root_name,string ee_name,KDL::Frame &T_world_geometry,bool reach_flag)
    {
  ...
  //----------------------------------------------------------------------------------------------------------          
   ...
   drc::traj_opt_constraint_t get_desired_hand_motion_traj_constraint(int64_t last_state_msg_timestamp, string robot_name,
...
//-------------------------------------------------------------------------------------------
 ...
 void get_endpose_search_constraints_from_sticky_hand(StickyHandStruc &handstruc,OtdfInstanceStruc& obj, KDL::Frame& T_world_body_desired, bool to_future_state,
                                                  bool end_state_only,
...
{
      string ee_name;
      if(!get_hand_palm_link_name(handstruc.hand_type,ee_name))
                                                  ...
                                                  double retracting_offset)
{
      string ee_name;
      if(!get_hand_palm_link_name(handstruc.hand_type,ee_name))
         cout << "unknown hand_type in get_motion_constraints_from_sticky_hand\n"; 
