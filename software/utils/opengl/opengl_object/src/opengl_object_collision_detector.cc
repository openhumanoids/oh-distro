  ...
  if( visible() ){
    ...
    if( _collision_detector != NULL ){
      ...
      for( unsigned int i = 0; i < collision_object_array.size(); i++ ){
        ...
        btCollisionShape * collision_shape = collision_object->getCollisionShape();
        btConvexHullShape * bt_convex_hull_shape = NULL;
        switch( collision_shape->getShapeType() ){
        case ( BOX_SHAPE_PROXYTYPE ):
          break;
        case ( CONVEX_HULL_SHAPE_PROXYTYPE ):
          bt_convex_hull_shape = dynamic_cast< btConvexHullShape* >( collision_shape );
          glBegin( GL_LINES );
        ...
        case ( SPHERE_SHAPE_PROXYTYPE ):
          _opengl_object_sphere.set( dynamic_cast< btSphereShape* >( collision_shape )->getRadius() );
          _opengl_object_sphere.draw( Vector3f( 1.0, 1.0, 0.0 ) );
          break;
        case ( CYLINDER_SHAPE_PROXYTYPE ):
          _opengl_object_cylinder.set( Vector2f( dynamic_cast< btCylinderShape* >( collision_shape )->getRadius(),
                                                  dynamic_cast< btConvexInternalShape* >( collision_shape )->getImplicitShapeDimensions().z() * 2 ) );
