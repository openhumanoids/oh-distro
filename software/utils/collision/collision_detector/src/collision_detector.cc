 ...
 * ray_test
 * performs a ray intersection test between two points and returns a pointer
 *   to the Collision_Object intersected and the point of intersection and normal
 */
void
Collision_Detector::
ray_test( Vector3f from,
  ...
  if( result.hasHit() ){
   ...
   hit_pt[1]=result.m_hitPointWorld[1];
   hit_pt[2]=result.m_hitPointWorld[2];
   hit_normal[0]=result.m_hitNormalWorld[0];
   hit_normal[1]=result.m_hitNormalWorld[1];
   hit_normal[2]=result.m_hitNormalWorld[2];
