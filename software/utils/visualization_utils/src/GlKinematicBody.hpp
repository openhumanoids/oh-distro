...
class GlKinematicBody
{
  ...
  //Avoids loading the same mesh multiple times.
   ...
   public:
    ...
    void run_fk_and_update_urdf_link_shapes_and_tfs(std::map<std::string, double> &jointpos_in, KDL::Frame &T_world_body);
    ...
    void run_fk_and_update_otdf_link_shapes_and_tfs(std::map<std::string, double> &jointpos_in, KDL::Frame &T_world_body);
    // void set_state(const drc::affordance_t &msg);    
    // NOTE: PROBLEM with set_state(const drc::affordance_t &msg)
