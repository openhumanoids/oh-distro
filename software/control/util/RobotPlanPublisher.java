...
public class RobotPlanPublisher
{
    ...
    public RobotPlanPublisher(String robot_name,String[] joint_name,String channel)
    {
    ...
    public RobotPlanPublisher(String robot_name, String[] joint_name,String channel, int num_states)
    {
      this(robot_name,joint_name,channel);
      allocate(num_states);
    }
...
*/        
        ...
        for(int j = 0;j<msg.plan[i].num_joints;j++)
        {
          msg.plan[i].joint_position[j] = (float) x[j][i];
          msg.plan[i].joint_velocity[j] = (float) x[msg.plan[i].num_joints+j][i];
