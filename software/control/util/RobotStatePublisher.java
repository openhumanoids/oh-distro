...
public class RobotStatePublisher
{
	...
	msg.angular_position = new float[msg.num_joints];
	msg.angular_velocity = new float[msg.num_joints];
	msg.measured_torque = new float[msg.num_joints];
	...
	for (int i=0; i<msg.num_joints; i++) {
	    msg.angular_position[i] = (float) x[i];
	    msg.angular_velocity[i] = (float) x[i+msg.num_joints];
	}
