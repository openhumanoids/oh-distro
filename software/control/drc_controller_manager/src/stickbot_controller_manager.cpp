...
#define NUM_OF_ARM_JOINTS 6
  ...
  void onUrdfMessage(const lcm::ReceiveBuffer* rbuf, const std::string& channel, const drc::robot_urdf_t* msg, boost::shared_ptr<drc_control::RobotModel> robot) 
  {
...
//constructor
  ...
  : _robot_name(robot_name),_lcm(lcm), _urdf_robot_model(robot_model) 
  {
    ...
    Gains.cart.kp_trans  = 200.0;
    Gains.cart.kd_trans = 60.0;  
...
//  Gains.cart.kp_trans  = 8.0;
//  Gains.cart.kd_trans = 2.0;  
//  Gains.cart.kp_rot = 1.0;  
//  Gains.cart.kd_rot =1e-2;
//  for (unsigned int i=0;i<NUM_OF_ARM_JOINTS;i++) {
//    Gains.joint.Kp_j[i]= 1e-4;
