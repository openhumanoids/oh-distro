    ...
    function obj=IRB140(urdf,options)
      ...
      if (~strcmp(options.hands, 'none'))
        ...
        elseif (strcmp(options.hands, 'block_hand'))
          ...
          options_hand.weld_to_link = 7;
          obj = obj.addRobotFromURDF([getenv('DRC_PATH'),'/models/IRB140/block_hand.urdf'], [0.1; 0.0; 0.0], [pi; 0; pi/2], options_hand);
        else
    ...
    function obj = compile(obj)
      ...
      if (obj.initialized)
        ...
        if (obj.hands > 0)
          ...
          arm_state_frame = replaceFrameNum(arm_state_frame,1,drcFrames.IRB140State(obj));
          ...
          arm_state_frame = getStateFrame(obj);
          if (length(arm_state_frame.coordinates) > length(drcFrames.IRB140State(obj).coordinates))
            arm_state_frame = replaceFrameNum(arm_state_frame, 1, drcFrames.IRB140State(obj));
          else
            arm_state_frame = drcFrames.IRB140State(obj);
          end
        end
        ...
        if (obj.hands > 0)
          ...
          input_frame  = replaceFrameNum(input_frame,1,drcFrames.IRB140Input(obj));
          % Sub in handstates for each hand
          % TODO: by name?
