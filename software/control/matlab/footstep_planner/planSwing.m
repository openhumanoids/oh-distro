...
if step_dist_xy > 0.01
  ...
  if nargin < 5
    ...
    apex_pos_l = [apex_lambda; apex_pos(3)];
  end
...
%   terrain_pts = [lambdas; biped.getTerrainHeight(lambda2xy.eval(lambdas))];
  terrain_pts = terrainSample(biped, last_pos, next_pos, contact_radius, 100, 10);
  
%   terrain_pts(2, isnan(terrain_pts(2,:))) = min([last_pos(3), next_pos(3)]);
