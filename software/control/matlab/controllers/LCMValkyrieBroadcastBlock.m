classdef LCMValkyrieBroadcastBlock < MIMODrakeSystem
  ...
  methods
    function obj = LCMValkyrieBroadcastBlock(r,r_control,options)
      ...
      obj.hokuyo_yaw_width = 0; %r.hokuyo_yaw_width;
      obj.hokuyo_num_pts = 0; %r.hokuyo_num_pts;
      obj.hokuyo_max_range = 0; %r.hokuyo_max_range;
      obj.hokuyo_spin_rate = 0; %r.hokuyo_spin_rate;
      ...
      if (isa(obj.getInputFrame, 'drcFrames.ValkyrieState'))
        valkyriecoordnames = obj.getInputFrame.getCoordinateNames;
      else
        valkyriecoordnames = obj.getInputFrame.getFrameByNameRecursive('drcFrames.ValkyrieState').getCoordinateNames;
      end
      valkyriecoordnames = valkyriecoordnames(7:length(valkyriecoordnames)/2); % cut off the floating base

      obj.joint_names_cache = cell(length(valkyriecoordnames), 1);
      % (this is the part that we really don't want to run repeatedly...)
      for i=1:length(valkyriecoordnames)
        obj.joint_names_cache(i) = java.lang.String(valkyriecoordnames(i));
      end

      obj.r = r;
      obj.r_control = r_control;
      obj.l_foot_id = obj.r_control.findLinkId(obj.r_control.l_foot_name);
      obj.r_foot_id = obj.r_control.findLinkId(obj.r_control.r_foot_name);
      obj.nq_control = obj.r_control.getNumPositions();
      if (isa(input_frame, 'MultiCoordinateFrame'))
        obj.frame_nums.valkyrie_state = input_frame.getFrameNumByNameRecursive('drcFrames.ValkyrieState');
        ...
        obj.frame_nums.valkyrie_state = 1;
    ...
    function varargout=mimoOutput(obj,t,~,varargin)
      ...
      num = obj.frame_nums.valkyrie_state;
      if isempty(num)
        error(['No valkyrie state found as input for LCMBroadcastBlock!']);
      elseif length(num)==1
        valkyrie_state = varargin{num};
      elseif length(num)==2
        valkyrie_state = varargin{num(1)};
        combined_frame = obj.getInputFrame.getFrameByNum(num(1));
        valkyrie_state = valkyrie_state(combined_frame.frame_id==num(2));
      else
      ...
      if (obj.frame_nums.left_foot_ft_state)
        ...
        com = obj.r_control.getCOM(valkyrie_state(1:obj.r_control.getNumPositions));
        kinsol = doKinematics(obj.r_control,valkyrie_state(1:obj.r_control.getNumPositions));
      ...
      num_dofs = length([valkyrie_state; right_hand_state; left_hand_state]) / 2;
      valkyrie_dofs = length(valkyrie_state)/2;
      ...
      % truth (on EST_ROBOT_STATE) or not (valkyrie_state)
      ...
      if (obj.publish_truth)
        % valkyrie_state has no global pose, but EST_ROBOT_STATE does
        ...
        state_msg.pose.translation.x = valkyrie_state(1);
        state_msg.pose.translation.y = valkyrie_state(2);
        state_msg.pose.translation.z = valkyrie_state(3);

        yaw = valkyrie_state(6);
        yaw = mod(yaw, 2*pi);
        if (yaw > pi)
          yaw = yaw - 2*pi;
        end
        q = rpy2quat([valkyrie_state(4) valkyrie_state(5) yaw]);
        ...
        state_msg.twist.linear_velocity.x = valkyrie_state(valkyrie_dofs+1);
        state_msg.twist.linear_velocity.y = valkyrie_state(valkyrie_dofs+2);
        state_msg.twist.linear_velocity.z = valkyrie_state(valkyrie_dofs+3);
        avel = rpydot2angularvel(valkyrie_state(4:6), valkyrie_state(valkyrie_dofs+4:valkyrie_dofs+6));
      ...
      valkyrie_pos = valkyrie_state(7:valkyrie_dofs);
      valkyrie_vel = valkyrie_state(valkyrie_dofs+7:end);
      if (~obj.publish_truth)
        state_msg.joint_position = valkyrie_pos;
        state_msg.joint_velocity = valkyrie_vel;
      else
        state_msg.joint_position = [valkyrie_pos; right_hand_state(1:right_hand_dofs); left_hand_state(1:left_hand_dofs); laser_spindle_angle];
        state_msg.joint_velocity = [valkyrie_vel; right_hand_state(right_hand_dofs+1:end); left_hand_state(left_hand_dofs+1:end); 0];
      end
      ...
      if (~obj.publish_truth)
        obj.lc.publish('valkyrie_state', state_msg);
      else
      ...
      pose_body_frame.pos = [valkyrie_state(1), valkyrie_state(2), valkyrie_state(3)];
      pose_body_frame.vel = [valkyrie_state(1+valkyrie_dofs), valkyrie_state(2+valkyrie_dofs), valkyrie_state(3+valkyrie_dofs)];
      for i=1:3
        valkyrie_state(3+i) = mod(valkyrie_state(3+i), 2*pi);
        if (valkyrie_state(3+i) > pi)
          valkyrie_state(3+i) = valkyrie_state(3+i) - 2*pi;
        end
      end
      q = rpy2quat([valkyrie_state(4) valkyrie_state(5) valkyrie_state(6)]);
      ...
      if (obj.publish_imu)
        this_state = [valkyrie_state(1:6); valkyrie_state(valkyrie_dofs+1:valkyrie_dofs+6)];
