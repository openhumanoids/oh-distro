classdef PosTorqueRefFeedthroughBlock < MIMODrakeSystem
  ...
  methods
    function obj = PosTorqueRefFeedthroughBlock(r,options)
      ...
      input_frame = MultiCoordinateFrame({atlasFrames.AtlasCoordinates(r),robot_input_frame});
      ...
      if isfield(options,'open_loop_torque')
        typecheck(options.open_loop_torque,'logical');
      else
      ...
      if options.open_loop_torque
        gains.k_f_p(arm_joints) = zeros(sum(arm_joints),1);
        gains.ff_qd(arm_joints) = zeros(sum(arm_joints),1);
        % scaling factor is made up here
        gains.ff_f_d(arm_joints) = 0.01*ones(sum(arm_joints),1);
      end        
        
%        % use relatively weak P gains for arms to pick up slack from model
%        % errors
       gains.k_q_p(arm_joints) = 0*ones(sum(arm_joints),1);
       gains.k_qd_p(arm_joints) = 0*ones(sum(arm_joints),1);
