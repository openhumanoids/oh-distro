classdef QPImpedanceController < MIMODrakeSystem

  methods
  function obj = QPImpedanceController(r,controller_data,options)
    ...
    obj.lhand_idx = findLinkInd(r,'l_hand+l_hand_point_mass');
    obj.rhand_idx = findLinkInd(r,'r_hand+r_hand_point_mass');
...
%    out_tic = tic;
    ...
    % use hands trajectory to get the goal position, 
    % CAUTION, the orientation should all be QUATERNION!
    % Also update the impedance stiffness and damping
    ...
    num_lhand_pts = size(lhand_pts,2);
    num_rhand_pts = size(rhand_pts,2);
    
    % Stiffness and damping are both 7x7 matrix
    K_imp_lhand = ctrl_data.lhand_stiffness; 
    D_imp_lhand = ctrl_data.lhand_damping;
    K_imp_rhand = ctrl_data.rhand_stiffness;
    D_imp_rhand = ctrl_data.rhand_damping;
    ...
    if (obj.use_mex==0 || obj.use_mex==2)
      ...
      [lhand_curr,J_lhand] = forwardKin(r,kinsol,obj.lhand_idx,lhand_pts,2);
      [rhand_curr,J_rhand] = forwardKin(r,kinsol,obj.rhand_idx,rhand_pts,2);

      lhand_err = zeros(7,num_lhand_pts);
      rhand_err = zeros(7,num_rhand_pts);
      lhand_err(1:3,:) = lhand_goal(1:3,:)-lhand_curr(1:3,:);
      rhand_err(1:3,:) = rhand_goal(1:3,:)-rhand_curr(1:3,:);

      lhand_quat_err_mask = sum(lhand_curr(4:7,:).*lhand_goal(4:7,:),1)>0;
      lhand_quat_err = lhand_quat_goal-lhand_curr(4:7,:).*bsxfun(@times,ones(4,1),lhand_quat_err_mask);
      lhand_err(4:7,:) = lhand_quat_err;
      rhand_quat_err_mask = sum(rhand_curr(4:7,:).*rhand_goal(4:7,:),1)>0;
      rhand_quat_err = rhand_quat_goal-rhand_curr(4:7,:).*bsxfun(@times,ones(4,1),rhand_quat_err_mask);
      rhand_err(4:7,:) = rhand_quat_err;
      lhand_vel = J_lhand*qd;
      rhand_vel = J_rhand*qd;
      
      impedance_torque = J_lhand'*(K_imp_lhand*lhand_err-D_imp_lhand*lhand_vel)+J_rhand'*(K_imp_rhand*rhand_err-D_imp_rhand*rhand_vel);
      ...
      if nq_free > 0
        beq_{1} = -C_con - H_con(:,obj.free_dof)*qdd_free+impedance_torque;
      else
        beq_{1} = -C_con+impedance_torque;
      end
