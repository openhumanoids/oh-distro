...
%     x_drill_reach = [.4;-.5;.4];            %% works for drilling .3 left sandia hand
    ...
    x_drill_reach = [.2;-.7;.7];            %% world position of drill reach
...
% x_drill_reach = [.3;-.6;.3]; 
[xtraj_reach,snopt_info_reach,infeasible_constraint_reach] = createInitialReachPlan(drill_pub, q0, x_drill_reach,first_cut_dir, 2);
...
%% line drill
x_drill_line = x_drill_in + 3*first_cut;
if use_simulated_state  
  q_drill_end = xtraj_drill.eval(xtraj_drill.tspan(end));
  q_drill_end = q_drill_end(1:r.num_q);
  q_drill_end = max(min(q_drill_end,ub),lb);
...
[xtraj_line,snopt_info_line,infeasible_constraint_line] = createDirectedLinePlan(drill_pub,q_drill_end, x_drill_line, 2);
