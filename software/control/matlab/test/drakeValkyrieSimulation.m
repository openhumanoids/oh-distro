function drakeValkyrieSimulation(visualize, add_hokuyo, right_hand, left_hand, world_name)
%NOTEST
if nargin < 1, visualize = false; end
if nargin < 2, add_hokuyo = true; end
if nargin < 3, right_hand = 0; end
if nargin < 4, left_hand = 0; end
...
r_complete = OHValkyrie([],options);

% simplify collision to parts we care about
r_pure = r_pure.removeCollisionGroupsExcept({'heel', 'toe', 'midfoot_front', 'midfoot_rear', 'feet'});
r_complete = r_complete.removeCollisionGroupsExcept({'heel', 'toe', 'midfoot_front', 'midfoot_rear', 'palm', 'knuckle', 'feet'});
...
while(~done)
  ...
  lcmInputBlock = LCMInputFromValkyrieCommandBlock(r_complete,r_pure,options);
  ...
  lcmBroadcastBlock = LCMValkyrieBroadcastBlock(r_complete,r_pure, broadcast_opts);
  for i=1:r_complete.getOutputFrame.getNumFrames
