function atlasPendulumControlTest
%NOTEST
...
if zero_crossing
	input_traj = PPTrajectory(spline(ts,joint_offset_map.(joint) + amp*sin(ts.*freq*2*pi)));
else
  input_traj = PPTrajectory(spline(ts, joint_offset_map.(joint) + joint_sign_map.(joint)*(0.5*amp - 0.5*amp*cos(ts.*freq*2*pi))));
end
ddtraj = fnder(input_traj,2);
