...
x_data_bkp = x_data;
for i=1:6,
  x_data(joint_indices(i),:) = unwrap(extra_data(16+i,:))-extra_data(16+i,1)+x_data(joint_indices(i),1);
end
...
t_sample = [299.2 311.1 336.9 362.5];
...
[dq, body1_params, body2_params, floating_states, residuals, info, J, body1_resids, body2_resids] = ...
  jointOffsetCalibration(r, q_data, joint_indices,torso_body,@(params) torsoMarkerPos_20131122(params,true), 0, torso_data, hand_body, @leftHandMarkerPos_20131122, 12, hand_data);
dq = unwrap([0;dq]);
dq = dq(2:end);
dq*180/pi
...
% q(3) = q(3) + 1;constructVisualizer
% q = q*0;
% q(6) = pi/2;
v.draw(0,q);
kinsol = r.doKinematics(q);
handpts = r.forwardKin(kinsol,hand_body,leftHandMarkerPos_20131122(body2_params));
