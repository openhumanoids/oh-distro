classdef ActionKinematicConstraint
  ...
  methods
    function obj = ActionKinematicConstraint(body_ind,body_pts,worldpos,tspan,name)
  ...
  methods (Static=true)
    function obj = onGroundConstraint(body_ind,body_pts,tspan,name)
      pos.min = repmat([nan;nan;0],1,size(body_pts,2));
      pos.max = repmat([nan;nan;0],1,size(body_pts,2));
      obj = ActionKinematicConstraint(body_ind,body_pts,pos,tspan,name);
    end
    
    function obj = groundConstraint(body_ind,body_pts,tspan,name)
        pos.min = repmat([nan;nan;0],1,size(body_pts,2));
