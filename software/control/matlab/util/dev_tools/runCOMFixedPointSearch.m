function xstar = runCOMFixedPointSearch()
...
comgoal = [mean(foot_pos(1:2,:),2);nan];
...
q = inverseKin(r,q0,0,comgoal,...
      rfoot_ind,[0;0;0],rfoot_pos,...
      lfoot_ind,[0;0;0],lfoot_pos,options);
...
figure(66);
plot(foot_pos(1,:),foot_pos(2,:),'bx','MarkerSize',10);
hold on;
plot(comgoal(1),comgoal(2),'gx','MarkerSize',10);
cm = r.getCOM(q);
