classdef ManipulationPlanner < handle
    ...
    methods
        ...
        function adjustAndPublishManipulationPlan(obj,x0,rh_ee_constraint,lh_ee_constraint)
            is_keyframe_constraint = true;
            lf_ee_constraint=[];rf_ee_constraint=[];
            runOptimization(obj,x0,rh_ee_constraint,lh_ee_constraint,rf_ee_constraint,lf_ee_constraint,is_keyframe_constraint);
        end
        
        function generateAndPublishManipulationPlan(obj,x0,rh_ee_goal,lh_ee_goal,rf_ee_goal,lf_ee_goal)
            is_keyframe_constraint = false;
            %rf_ee_goal=[];lf_ee_goal=[];
            runOptimization(obj,x0,rh_ee_goal,lh_ee_goal,rf_ee_goal,lf_ee_goal,is_keyframe_constraint);
        ...
        function runOptimization_old(obj,x0,r_ee_goal,l_ee_goal,is_keyframe_constraint)
            disp('Generating plan...');
            send_status(3,0,0,'Generating manip plan...');%3 is p&c, 4 is grasping
            ...
            %%%+============================
            kc_rfoot = ActionKinematicConstraint(obj.r,rfoot_body,[0;0;0],r_foot_pose0,[s(1),s(end)],'rfoot0');
            ks = ks.addKinematicConstraint(kc_rfoot);
            kc_lfoot = ActionKinematicConstraint(obj.r,lfoot_body,[0;0;0],l_foot_pose0,[s(1),s(end)],'lfoot0');
            ks = ks.addKinematicConstraint(kc_lfoot);
            ...
            if(~is_keyframe_constraint)
                ...
                [q_final_guess,snopt_info] = inverseKin(obj.r,q0,...
                    pelvis_body,[0;0;0],pelvis_pose0,...
                    utorso_body,[0;0;0],utorso_pose0_relaxed,...
                    rfoot_body,[0;0;0],r_foot_pose0, ...
                    lfoot_body,[0;0;0],l_foot_pose0, ...
                    r_hand_body,[0;0;0],rhand_const, ...
                    l_hand_body,[0;0;0],lhand_const,...
                    ikoptions);
            ...
            for i=1:length(affIndicesTimes),
                ...
                for k=1:length(ind),
                    ...
                    if(strcmp('left_palm',ee_names{ind(k)}))
                        ...
                        plan_affIndices(i).dof_name=[plan_affIndices(i).dof_name;affIndices(i).dof_name(1)];
                        plan_affIndices(i).dof_value=[plan_affIndices(i).dof_value;affIndices(i).dof_value(1)];
                    end
                    ...
                    if(strcmp('l_foot',ee_names{ind(k)}))
                        ...
                        lfoot_const.min = l_foot_pose-1e-4*[ones(3,1);ones(4,1)];
                        lfoot_const.max = l_foot_pose+1e-4*[ones(3,1);ones(4,1)];
                        plan_affIndices(i).num_ees=plan_affIndices(i).num_ees+1;
                    ...
                    if(strcmp('r_foot',ee_names{ind(k)}))
                        ...
                        rfoot_const.min = r_foot_pose-1e-4*[ones(3,1);ones(4,1)];
                        rfoot_const.max = r_foot_pose+1e-4*[ones(3,1);ones(4,1)];
                        plan_affIndices(i).num_ees=plan_affIndices(i).num_ees+1;
                ...
                [q(:,i),snopt_info] = inverseKin(obj.r,q_guess,...
                    pelvis_body,[0;0;0],pelvis_pose0,...
                    utorso_body,[0;0;0],utorso_pose0_relaxed,...
                    rfoot_body,'heel',rfoot_const, ...
                    lfoot_body,'heel',lfoot_const, ...
                    r_hand_body,[0;0;0],rhand_const, ...
                    l_hand_body,[0;0;0],lhand_const,...
                    ikoptions);
                q_guess =q(:,i);
        ...
        function runOptimization(obj,x0,rh_ee_goal,lh_ee_goal,rf_ee_goal,lf_ee_goal,is_keyframe_constraint)
            disp('Generating plan...');
            ...
            if(~is_keyframe_constraint)
                ...
                if(isempty(lf_ee_goal))
                    ...
                    s_int_lf = rf_ee_goal.time*obj.time_2_index_scale;
                    % Desired position of palm in world frame
            ...
            s_total = max(max(s_total_lh,s_total_rh),max(s_total_lf,s_total_rf));
            ...
            for i=2:length(s)
                ...
                l_foot_pose_at_t=pose_spline(s_breaks,lfoot_breaks,si); % evaluate in quaternions
                %           r_hand_pose_at_t=pose_foh(s_breaks,rhand_breaks,si); % evaluate in quaternions
                %           l_hand_pose_at_t=pose_foh(s_breaks,lhand_breaks,si); % evaluate in quaternions
                ...
                if(si~=s(end))
                    ...
                    lfoot_const.max = l_foot_pose_at_t+1e-2*[ones(3,1);2*ones(4,1)];
                else
                    rhand_const.min = r_hand_pose_at_t-1e-4*[ones(3,1);ones(4,1)];
                    rhand_const.max = r_hand_pose_at_t+1e-4*[ones(3,1);ones(4,1)];
                    lhand_const.min = l_hand_pose_at_t-1e-4*[ones(3,1);ones(4,1)];
                    ...
                    lfoot_const.max = l_foot_pose_at_t+1e-4*[ones(3,1);2*ones(4,1)];
                end
                %l_pose_constraint=[nan;nan;nan;quat2rpy(l_hand_pose_at_t(4:7))];
                %l_pose_constraint=[nan;nan;nan;lhandT(4);lhandT(5);lhandT(6)];
                %l_pose_constraint=[lhandT(1);lhandT(2);lhandT(3);lhandT(4);lhandT(5);lhandT(6)];
