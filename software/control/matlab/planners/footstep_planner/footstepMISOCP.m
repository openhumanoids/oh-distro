...
sector_width = pi/4;
sector_boundaries = -2*pi:sector_width:2*pi;
...
foci = [[.35;-0.26], [-.35;-0.26]];
ellipse_l = sum(sqrt(sum(diff(foci, [], 2).^2))) * 1.03;
...
for j = 1:nsteps
  for s = 1:length(sector_boundaries) - 1
    th0 = sector_boundaries(s);
    th1 = sector_boundaries(s+1);
    
    cos_slope = (cos(th1) - cos(th0)) / (sector_width);
    cos_intercept = cos(th0) - (cos_slope * th0);
    sin_slope = (sin(th1) - sin(th0)) / (sector_width);
    sin_intercept = sin(th0) - (sin_slope * th0);
    
    Constraints = [Constraints,...
                   implies(sector(s, j), th0 <= yaw(j) <= th1),...
                   implies(sector(s, j), cos_yaw(j) == cos_slope * yaw(j) + cos_intercept),...
                   implies(sector(s, j), sin_yaw(j) == sin_slope * yaw(j) + sin_intercept)];
  end
