...
while true
  ...
  switch ctrl_type
    case drc.drill_control_t.LADDER_STRAIGHTEN_LEFT
      ...
      if ~isempty(ladder)
        [xtraj, snopt_info, infeasible_constraint] = straightenLeftHand(obj, q0, ladder.forward);
      else
    ...
    case drc.drill_control_t.LADDER_STRAIGHTEN_RIGHT
      ladder = lcm_mon.getLadderAffordance();
      if ~isempty(ladder)
        [xtraj, snopt_info, infeasible_constraint] = straightenRightHand(obj, q0, ladder.forward);
      else
        send_status(4,0,0,'No ladder found, cannot straighten left hand');
      end
  end
