...
//#define USE_J_PINV_CONTROLLER  // requires Weighted Jacobian and Damped Pinv to work.
  ...
  struct JointGains
  {
   Eigen::Matrix<double, JOINTS, 1> Kp_j;
   Eigen::Matrix<double, JOINTS, 1> Kd_j;
   Eigen::Matrix<double, JOINTS, 1> Kff_j;
  };
  
  template <int JOINTS>
  struct PDChainControlGains
  {
   CartesianGains cart;
   JointGains<JOINTS> joint;
   NullspaceBiasGains nullspace;
  };
...
// pd_chain_control::ChainController<NrOfJoints> left_arm_controller(lcm,"LEFT_ARM_CMDS",kdl_chain);
  ...
  public:
    ...
    //  _use_J_Transpose_Controller replicates ROS's IK for PR2 use Jtranspose method
    // if false; uses Jpinv method
    // requires Weighted Jacobian and Damped Pinv to work.
    // Implementation is based on the following paper (+ NULL SPACE biasing)
    // T. Sugihara, "Solvability-Unconcerned Inverse Kinematics
    // by the Levenberg-Marquardt method", IEEE Transactions on Robotics, 
    // Vol. 27, No. 5, 2011.  
    ChainController(boost::shared_ptr<lcm::LCM> &lcm, const std::string &input_cmds_channel,
			 const KDL::Chain &chain, const std::string &robot_name,const urdf::Model &urdf_robot_model,const PDChainControlGains<JOINTS> Gains,const bool _use_J_Transpose_Controller);
    ~ChainController();
  ...
  bool debug_;
  bool _use_J_Transpose_Controller;
...
template <int JOINTS>
  ChainController<JOINTS>::ChainController(boost::shared_ptr<lcm::LCM> &lcm, 
	  ...
	  const urdf::Model &urdf_robot_model,
	  const PDChainControlGains<JOINTS> Gains,
	  const bool use_J_T_Controller) : kdl_chain(chain), _lcm(lcm), _robot_name(robot_name), _urdf_robot_model(urdf_robot_model),_use_J_Transpose_Controller(use_J_T_Controller)
  {
   ...
   controller_state = STOPPED;
    ...
    for (int i = 0; i < JOINTS; ++i){
      Kd_j[i]  = Gains.joint.Kd_j[i];
      Kp_j[i]  = Gains.joint.Kp_j[i];
      Kff_j[i] = Gains.joint.Kff_j[i];
    }

    kp_trans  = Gains.cart.kp_trans;
    kd_trans = Gains.cart.kd_trans;  
    kp_rot = Gains.cart.kp_rot;    
    kd_rot =Gains.cart.kd_rot;
    k_posture_ = Gains.nullspace.k_posture_;
    k_posture_dot_ = Gains.nullspace.k_posture_dot_;
  ...
  //  q_measured_<<0,0,0,0,0*(M_PI/180),0*(M_PI/180);
    ...
    if(_use_J_Transpose_Controller)
     {
    ...
    Eigen::MatrixXd S = Eigen::MatrixXd::Zero(6,JOINTS);
    VectorXq sigma = svd.singularValues();
    for (unsigned int i=0; i<sigma.size();i++)
       S(i,i) = sigma(i);
    /*std::cout <<"J" <<J << std::endl;
    std::cout <<"Jr" <<(U*S*V.transpose())<< std::endl;*/
    /*std::cout <<"Vsize" <<V.rows()<<"x"<<V.cols()  << std::endl;
    std::cout <<"Ssize" <<S.rows()<<"x"<<S.cols()  << std::endl;
    std::cout <<"Usize" <<U.rows()<<"x"<<U.cols()  << std::endl;*/

    Eigen::MatrixXd ST = Eigen::MatrixXd::Zero(JOINTS,6);
    for (unsigned int i=0; i<original_Aii.size();i++)
       ST(i,i) = original_Aii(i);//1/sigma(i);

    Eigen::Matrix<double,JOINTS,6> J_pinv_damped;
    J_pinv_damped =  V*ST*U.transpose();
    ...
    Eigen::Matrix<double,6,6> JJt_inv_damped;
    JJt_inv_damped = JJt_damped.inverse();
    Eigen::Matrix<double,JOINTS,6> J_pinv_damped2 = J.transpose() * JJt_inv_damped;

    std::cout <<"J_pinv_damped" <<J_pinv_damped << std::endl;
    std::cout <<"J_pinv_damped2" <<J_pinv_damped2 << std::endl;*/

    //testing svd
    /*Eigen::Matrix<double,9,6> T = Eigen::Matrix<double,9,6>::Random(9,6);
        Eigen::JacobiSVD<Eigen::MatrixXd> svd2; 
    svd2.compute(T, Eigen::ComputeThinU | Eigen::ComputeThinV);
    std::cout <<"TEST:"<< std::endl;
    std::cout <<"Vsize" <<svd2.matrixV().rows()<<"x"<<svd2.matrixV().cols()  << std::endl;
    std::cout <<"sing vals" <<svd2.singularValues().size() << std::endl;
    std::cout <<"Usize" <<svd2.matrixU().rows()<<"x"<<svd2.matrixU().cols() << std::endl;

    Eigen::MatrixXd V2 = svd2.matrixV();
    Eigen::MatrixXd S2 = Eigen::MatrixXd::Zero(9,6);
    VectorXq sigma = svd2.singularValues();
    for (unsigned int i=0; i<sigma.size();i++)
       S2(i,i) = sigma(i);
    ...
    std::cout <<"T_recon" <<(V2*S2*U2.transpose()).transpose() << std::endl;
    std::cout <<"T_recon" <<(U2*S2*V2.transpose()) << std::endl;
    */
    ...
    // DO NOT SET MSG TIMESTAMP HERE
    // Timestamp is set in controller_manager to latest robot state msg timestamp since
    // the actuation command is in response to the latest known robot state.
    // Time stamp is set in controller_manager after actuation commands from multiple controllers are 
    // collated.
    // 
    torque_cmd.utime = 0;    
    ...
    for(int i = 0; i < torque_cmd.num_actuators; i++){
      ...
      torque_cmd.effort_duration.push_back(0.05);// expires after 0.05 sec
    }
