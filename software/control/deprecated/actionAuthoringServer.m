...
robot_state_coder = LCMCoordinateFrame('drcFrames.AtlasState',JLCMCoder(drc.control.RobotStateCoder(joint_names)),'x');
...
while (1)
    warning on
    if ~isempty(newdata)
        fprintf('Interrupted, loading new data...\n');
        while (1)
    ...
    if ~isempty(data)
        ...
        try
            ...
            if(action_options.IK)
                ...
                %    'RESPONSE_IK_SOLUTION_AT_TIME_FOR_ACTION_SEQUENCE');
                % above isnt working, so publishing as a plan of length
                %   one instead...
                publish(robot_plan_publisher_oneshot, ik_time, ...
                    x);
...
if(goal.contact_type == goal.COLLISION_AVOIDANCE)
    ...
    offsetl = [goal.x_offset_lower; goal.y_offset_lower; goal.z_offset_lower];
    offsetu = [goal.x_offset_upper; goal.y_offset_upper; goal.z_offset_upper];
    pos = struct();
    pos.max = inf(3,1);
    pos.min = -inf(3,1);
    pl = p + offsetl;
    pu = p + offsetu;
    if(goal.x_relation_lower == 0)
        pos.min(1) = pl(1);
        pos.max(1) = pl(1);
    elseif(goal.x_relation_lower == 1)
        pos.max(1) = pl(1);
    elseif(goal.x_relation_lower == 2)
        pos.min(1) = pl(1);
    elseif(goal.x_relation_lower == 3)
        ;
    end
    if(goal.x_relation_upper == 0)
        pos.min(1) = pu(1);
        pos.max(1) = pu(1);
    elseif(goal.x_relation_upper == 1)
        pos.max(1) = pu(1);
    elseif(goal.x_relation_upper == 2)
        pos.min(1) = pu(1);
    elseif(goal.x_relation_upper == 3)
        ;
    end
    if(goal.y_relation_lower == 0)
        pos.min(2) = pl(2);
        pos.max(2) = pl(2);
    elseif(goal.y_relation_lower == 1)
        pos.max(2) = pl(2);
    elseif(goal.y_relation_lower == 2)
        pos.min(2) = pl(2);
    elseif(goal.y_relation_lower == 3)
        ;
    end
    if(goal.y_relation_upper == 0)
        pos.min(2) = pu(2);
        pos.max(2) = pu(2);
    elseif(goal.y_relation_upper == 1)
        pos.max(2) = pu(2);
    elseif(goal.y_relation_upper == 2)
        pos.min(2) = pu(2);
    elseif(goal.y_relation_upper == 3)
        ;
    end
    if(goal.z_relation_lower == 0)
        pos.min(3) = pl(3);
        pos.max(3) = pl(3);
    elseif(goal.z_relation_lower == 1)
        pos.max(3) = pl(3);
    elseif(goal.z_relation_lower == 2)
        pos.min(3) = pl(3);
    elseif(goal.z_relation_lower == 3)
        ;
    end
    if(goal.z_relation_upper == 0)
        pos.min(3) = pu(3);
        pos.max(3) = pu(3);
    elseif(goal.z_relation_upper == 1)
        pos.max(3) = pu(3);
    elseif(goal.z_relation_upper == 2)
        pos.min(3) = pu(3);
    elseif(goal.z_relation_upper == 3)
        ;
    end
