...
MatrixXd GetTaskSpaceJacobian(const RigidBodyTree& r,
  ...
  MatrixXd Jg = r.geometricJacobian(cache, 0, body.body_index, 0, true,
                                    &v_or_q_indices);
  MatrixXd J(6, r.num_velocities);
  J.setZero();

  Vector3d points =
      r.transformPoints(cache, local_offset, body.body_index, 0);

  int col = 0;
  for (auto it = v_or_q_indices.begin(); it != v_or_q_indices.end(); ++it) {
    // angular part
    J.template block<3, 1>(0, *it) =
        Jg.block<3, 1>(0, col);
    // linear part
    // Assuming joint velocity = 1, the column is the linear velocity.
    J.template block<3, 1>(3, *it) =
        Jg.block<3, 1>(3, col);
    J.template block<3, 1>(3, *it).noalias() +=
