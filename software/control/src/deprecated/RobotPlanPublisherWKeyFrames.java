...
public class RobotPlanPublisherWKeyFrames
{
    ...
    public RobotPlanPublisherWKeyFrames(String robot_name,String[] joint_name,boolean floating_base, String channel, int num_keyframes)
    {
      msg = new drc.robot_plan_w_keyframes_t();
      msg.robot_name = robot_name;
      channel_name = channel;
      has_floating_base = floating_base;
      no_keyframes = num_keyframes;
      nonfloating_joint_start_ndx = 0;
      //System.out.println(has_floating_base);
      if (has_floating_base)
        nonfloating_joint_start_ndx = 7;
      int num_nonfloating_joints = joint_name.length - nonfloating_joint_start_ndx+1;
      nonfloating_joint_name = new String[num_nonfloating_joints];
      for (int i=nonfloating_joint_start_ndx-1,j=0; i<joint_name.length; i++)
    ...
    public RobotPlanPublisherWKeyFrames(String robot_name, String[] joint_name,boolean has_floating_base, String channel, int num_keyframes, int num_states)
    {
      this(robot_name,joint_name,has_floating_base,channel,num_keyframes);
      allocate(num_states,num_keyframes);
    }

    public void allocate(int num_states, int num_keyframes)
    {
      msg.num_states = num_states;
      msg.num_keyframes = num_keyframes;
      msg.is_keyframe = new boolean[msg.num_states];
    ...
    public LCMEncodable encode(double[] t, double[][] x)
    {
      ...
      {
        if (has_floating_base) {
          msg.is_keyframe[i] = (boolean) (x[0][i] == 1);
          msg.plan[i].utime = (long) (t[i]*1000000+msg.utime);        
          msg.plan[i].origin_position.translation.x = (float) x[1][i];
          msg.plan[i].origin_position.translation.y = (float) x[2][i];
          msg.plan[i].origin_position.translation.z = (float) x[3][i];
          ...
          msg.plan[i].origin_twist.linear_velocity.x = x[7+msg.plan[i].num_joints][i];
          msg.plan[i].origin_twist.linear_velocity.y = x[7+msg.plan[i].num_joints+1][i];
          msg.plan[i].origin_twist.linear_velocity.z = x[7+msg.plan[i].num_joints+2][i];
          msg.plan[i].origin_twist.angular_velocity.x = x[7+msg.plan[i].num_joints+3][i];
          msg.plan[i].origin_twist.angular_velocity.y = x[7+msg.plan[i].num_joints+4][i];
          msg.plan[i].origin_twist.angular_velocity.z = x[7+msg.plan[i].num_joints+5][i];
    ...
    public void publish(double[] t,double[][] x)
    {
      if (t.length != msg.num_states) {
        allocate(t.length,no_keyframes);  // reallocate on-demand
//        System.out.format("Error: Publish failed. RobotPlanPublisherWKeyFrames was initialized with %d states, but trying to publish %d.\n",msg.num_states,t.length);
//        return;
      }
