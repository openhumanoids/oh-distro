...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  if (mxGetScalar(prhs[0])==0) { // then construct the data object and return
    ...
    error = GRBnewmodel(pdata->env,&(pdata->model),"QPController",0,NULL,NULL,NULL,NULL,NULL);
  ...
  int num_vars = mxGetN(A), num_constraints = mxGetM(A), i;

  if (num_vars != pdata->num_vars) {
    // modify number of vars
    if (num_vars > pdata->num_vars) {
      error = GRBaddvars(pdata->model,num_vars-pdata->num_vars,0,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL);
    } else {
      int count=pdata->num_vars - num_vars;
      error = GRBdelvars(pdata->model,count,const_cast<int*>(&nonnegative_ints[num_vars]));
    }
    pdata->num_vars = num_vars;
  }
  
  error = GRBupdatemodel(pdata->model);
  
  // reset the obj,lb,ub
  error = GRBsetdblattrarray(pdata->model,"Obj",0,num_vars,mxGetPr(obj));
  error = GRBsetdblattrarray(pdata->model,"LB",0,num_vars,mxGetPr(lb));
  error = GRBsetdblattrarray(pdata->model,"UB",0,num_vars,mxGetPr(ub));

  error = GRBupdatemodel(pdata->model);
  
  error = GRBdelconstrs(pdata->model,pdata->num_constraints,const_cast<int*>(nonnegative_ints));

  if (num_constraints != pdata->num_constraints) {
    // delete constraints
    // add constraints
    pdata->num_constraints = num_constraints;
  } else {
    // modify constraints
  }
  ...
  { // Ax = rhs constraints
    ...
    int *cbeg, *cval;
    char *s = new char[num_constraints]; 
    mxGetString(sense,s,num_constraints);

    if (sizeof(int)==sizeof(mwIndex)) {     // avoid ugly copies if possible
      cbeg=reinterpret_cast<int*>(jc);
      cval=reinterpret_cast<int*>(ir);
    } else {
      cbeg = new int[num_constraints]; cval = new int[nnz];
      for (i=0; i<num_constraints; i++) cbeg[i] = (int) jc[i];
      for (i=0; i<nnz; i++) cval[i] = (int) ir[i];
    }

    error = GRBaddconstrs(pdata->model,num_constraints,nnz,cbeg,cval,mxGetPr(At),s,mxGetPr(rhs),NULL);	
  }

  // delete qpterms
  error = GRBdelq(pdata->model);
  ...
  { // add quadratic cost terms
    ...
    if (sizeof(int)==sizeof(mwIndex)) {  // avoid ugly copy if possible
      ...
      error = GRBaddqpterms(pdata->model,nnz,reinterpret_cast<int*>(ir),qcol,mxGetPr(Q));
    } else {
      ...
      error = GRBaddqpterms(pdata->model,nnz,qrow,qcol,mxGetPr(Q));
    }
  }
  
  error = GRBupdatemodel(pdata->model);
  error = GRBoptimize(pdata->model);

  if (nlhs>0) {
    plhs[0] = mxCreateDoubleMatrix(num_vars,1,mxREAL);
    error = GRBgetdblattrarray(pdata->model, GRB_DBL_ATTR_X, 0, num_vars, mxGetPr(plhs[0]));
  }
