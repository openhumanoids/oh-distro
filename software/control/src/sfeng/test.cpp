...
QPOutput testGravityCompensation(const sfRobotState &rs)
{
  ...
  if (!input.loadParamFromFile(std::string(drc_path) + std::string("/software/control/src/sfeng/config/qpc_params"))) {
    throw std::runtime_error("cant load qp controller config file");
  }
...
void testQPEstimator(const std::string &urdf, const sfRobotState &rs, const VectorXd &q, const VectorXd &qd, const VectorXd &trq, const Vector6d &ft_l, const Vector6d &ft_r)
{
  ...
  if (!est.loadParamFromFile(std::string(drc_path) + std::string("/software/control/src/sfeng/config/qpe_params"))) {
    throw std::runtime_error("cant load qp estimator config file");
  }
...
int main()
{
  ...
  std::string urdf = std::string(drc_path) + std::string("/software/models/val_description/urdf/valkyrie_sim_drake.urdf");
  sfRobotState rs(std::unique_ptr<RigidBodyTree>(new RigidBodyTree(urdf, DrakeJoint::ROLLPITCHYAW)));

  ////////////////////////////////////////////////////////////////////
  // set state and do kinematics
  VectorXd q(rs.robot->num_positions);
  //VectorXd q(rs.robot->number_of_positions());
  VectorXd qd(rs.robot->num_velocities);
  ...
  rs.update(0, q, qd, VectorXd::Zero(rs.robot->num_velocities-6), Vector6d::Zero(), Vector6d::Zero());
