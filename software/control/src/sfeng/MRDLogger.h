...
public:
  ...
  // Eigen Vector channel
  template <typename Scalar, size_t n> bool addChannel(std::vector<std::string> &names, std::vector<std::string> &units, const Eigen::Matrix<Scalar, n, 1> &V)
  {
    if (n != names.size() || names.size() != units.size()) {
      throw std::runtime_error("MRDLogger can't add Eigen Vector due to size mismatch.");
    }
    bool ret = true;
    for (size_t i = 0; i < n; i++) {
      ret &= addChannel(names[i], units[i], V.data()+i);
      if (!ret)
        break;
    }
    return ret;
