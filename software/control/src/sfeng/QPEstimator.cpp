...
void QPEstimator::addToLog(MRDLogger &logger) const
{
  for (int i = 0; i < residual.size(); i++)
    logger.addChannel("QPE.err["+rs.robot->getPositionName(i)+"]", "-", residual.data()+i);
  for (int i = 0; i < vel.size(); i++)
    logger.addChannel("QPE.v["+rs.robot->getPositionName(i)+"]", "rad/s", vel.data()+i);
  for (int i = 0; i < trq.size(); i++)
    logger.addChannel("QPE.trq["+rs.robot->getPositionName(i+6)+"]", "Nm", trq.data()+i); 

  logger.addChannel("QPE.F_w[L][x]", "N", foot_wrench_in_sensor_frame[Side::LEFT].data()+3);
  logger.addChannel("QPE.F_w[L][y]", "N", foot_wrench_in_sensor_frame[Side::LEFT].data()+4);
  logger.addChannel("QPE.F_w[L][z]", "N", foot_wrench_in_sensor_frame[Side::LEFT].data()+5);
  logger.addChannel("QPE.M_w[L][x]", "Nm", foot_wrench_in_sensor_frame[Side::LEFT].data()+0);
  logger.addChannel("QPE.M_w[L][y]", "Nm", foot_wrench_in_sensor_frame[Side::LEFT].data()+1);
  logger.addChannel("QPE.M_w[L][z]", "Nm", foot_wrench_in_sensor_frame[Side::LEFT].data()+2);
  
  logger.addChannel("QPE.F_w[R][x]", "N", foot_wrench_in_sensor_frame[Side::RIGHT].data()+3);
  logger.addChannel("QPE.F_w[R][y]", "N", foot_wrench_in_sensor_frame[Side::RIGHT].data()+4);
  logger.addChannel("QPE.F_w[R][z]", "N", foot_wrench_in_sensor_frame[Side::RIGHT].data()+5);
  logger.addChannel("QPE.M_w[R][x]", "Nm", foot_wrench_in_sensor_frame[Side::RIGHT].data()+0);
  logger.addChannel("QPE.M_w[R][y]", "Nm", foot_wrench_in_sensor_frame[Side::RIGHT].data()+1);
  logger.addChannel("QPE.M_w[R][z]", "Nm", foot_wrench_in_sensor_frame[Side::RIGHT].data()+2);
}
...
{
  if (q.size() != rs.robot->num_positions || 
      v.size() != rs.robot->num_velocities || 
      trq.size() != rs.robot->actuators.size()) {
    throw std::runtime_error("robot state dimension mismatch");
  }

  int nContacts = 2;
  int nQdd = rs.robot->num_velocities;
  //int nQdd = rs.robot->number_of_velocities();
  int nWrench = 6 * nContacts;
  int nTrq = nQdd - 6;
  if (nTrq != rs.robot->actuators.size()) {
    throw std::runtime_error("qp controller torque size mismatch (probably not floating  base model)");
  }
  ...
  // tau = rs.robot->B.bottomRows(nTrq) * u, tau is joint space indexed, u is actuator space indexed.
  // constraints are specified with u index. 
  _CI.block(rowIdx,0,nTrq,_nVar) = rs.robot->B.bottomRows(nTrq).transpose() * Tau; 
  _ci_l.segment(rowIdx,nTrq) = -rs.robot->B.bottomRows(nTrq).transpose() * tau0;
  _ci_u.segment(rowIdx,nTrq) = -rs.robot->B.bottomRows(nTrq).transpose() * tau0;
  ...
  */ 
  ...
  this->foot_wrench_in_sensor_frame[Side::LEFT] = _X.segment(lambda_start, 6);
  this->foot_wrench_in_sensor_frame[Side::RIGHT] = _X.segment(lambda_start+6, 6);
