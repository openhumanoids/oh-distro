...
void Pass::robot_state_handler(const lcm::ReceiveBuffer* rbuf, const std::string& channel, const  drc::robot_state_t* msg){
  ...
  */
  ...
  body_to_aff.translation()  << 0.45, 0.1, 0.3;
  Eigen::Quaterniond quat2 = Eigen::Quaterniond(  euler_to_quat( 90*M_PI/180 ,  0*M_PI/180 ,70*M_PI/180  )  );
  body_to_aff.rotate(quat2);    

  Eigen::Isometry3d world_to_aff = world_to_body_*body_to_aff; 
  
  std::vector<double> xyzrpy0 = {.0 , .0 , .0, 0. , 0 , 0};
  
  xyzrpy0[0] = world_to_aff.translation().x();
  xyzrpy0[1] = world_to_aff.translation().y();
  xyzrpy0[2] = world_to_aff.translation().z();
  quat_to_euler ( Eigen::Quaterniond(world_to_aff.rotation() ), xyzrpy0[3], xyzrpy0[4], xyzrpy0[5] );
  
  
  int uid0 = 1;
  string filename0 = "notused";
  drc::affordance_plus_t a0 = getSteeringCylinderAffordancePlus(filename0, xyzrpy0, uid0);
