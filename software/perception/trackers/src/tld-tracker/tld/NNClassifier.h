...
public:
	...
	void release();
	float classifyPatch(NormalizedPatch * patch, int* pidx=0, int* nidx=0);
	float classifyBB(Mat img, Rect* bb, int* pidx=0, int* nidx=0);
	float classifyWindow(Mat img, int windowIdx, int* pidx=0, int* nidx=0);
	void learn(vector<NormalizedPatch> patches);
	bool filter(Mat img, int windowIdx, int* pidx=0, int* nidx=0);
};
