...
float NNClassifier::classifyPatch(NormalizedPatch * patch, int* pidx, int* nidx) {
...
float NNClassifier::classifyBB(Mat img, Rect* bb, int* pidx, int* nidx) {
	NormalizedPatch patch;

	tldExtractNormalizedPatchRect(img, bb, patch.values);
	return classifyPatch(&patch, pidx, nidx);

}

float NNClassifier::classifyWindow(Mat img, int windowIdx, int* pidx, int* nidx) {
	NormalizedPatch patch;
...
bool NNClassifier::filter(Mat img, int windowIdx, int* pidx, int* nidx) {
	if(!enabled) return true;

	float conf = classifyWindow(img, windowIdx, pidx, nidx);
