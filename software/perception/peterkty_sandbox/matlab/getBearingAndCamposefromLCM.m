...
while true
    ...
    millis_to_wait = 1000;
    bmsg = aggregator.getNextMessage(millis_to_wait);
    if ~isempty(bmsg)
        break
    end
end
...
bmsg_dec = perception.pointing_vector_t(bmsg.data);
yk = atan([bmsg_dec.vec(1), bmsg_dec.vec(2)])';
fprintf('bearing angle(rad): %f %f\n', yk(1), yk(2));
