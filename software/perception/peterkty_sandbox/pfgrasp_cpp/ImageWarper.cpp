...
struct ImageWarper {
  ...
  void unWarp(const cv::Mat in, const cv::Mat out) {
    cv::Mat in;
    int imgType = CV_8UC3;   //??? should be from "in"
  ...
  bool setChannels(const std::string& iInputChannel,
    ...
    BotCamTrans* outputCamTrans =
      bot_param_get_new_camtrans(mBotWrapper->getBotParam(),
                                 mOutputChannel.c_str());   // initialize an identity transform
    ...
    int inputHeight = in.Heigth();
    mOutputWidth = out.Width();
    mOutputHeight = out.Heigth();
