function result = optimize_spindle_pose(data1,data2,poses_start1,poses_end1,...
    poses_start2, poses_end2,P_camera_to_pre_spindle,P_post_spindle_to_lidar,do_full)
...
x_full = poses_to_vector(P_camera_to_pre_spindle, P_post_spindle_to_lidar, do_full);
...
prob.P_camera_to_pre_spindle = P_camera_to_pre_spindle;
prob.P_post_spindle_to_lidar = P_post_spindle_to_lidar;
...
[result.P_camera_to_pre_spindle, result.P_post_spindle_to_lidar] = vector_to_poses([prob.x_cam(:);x(:)],do_full);
...
x_full = [prob.x_cam(:); x(:)];
[P_camera_to_pre_spindle, P_post_spindle_to_lidar] = vector_to_poses(x_full,prob.do_full);

pts1 = accum_lidar(prob.data1, prob.poses_start1, prob.poses_end1,...
    P_camera_to_pre_spindle,P_post_spindle_to_lidar,false);
