...
public:
  ...
  int getNActiveVoxels()
  {
    return count(active_voxels.begin(), active_voxels.end(), 1);
  }
  int getNActiveOrientations()
  {
    return this->active_orientations.count();
  }
  Eigen::Vector3d getMapCentre()
  {
    return map_centre;
  }
  Eigen::Vector3d getMapUpperBound()
  {
    return map_upper_bound;
  }
  Eigen::Vector3d getMapLowerBound()
  {
    return map_lower_bound;
  }
  std::string getBaseLink()
  {
    return base_link;
  }
  Eigen::Vector3d getOrientation(int orient)
  {
    return occupancy_map_orientations[orient];
  }
  Eigen::Vector3d getVoxelCentre(int vox)
  {
    return voxel_centres[vox];
  }
  std::vector<Eigen::Vector3d> getVoxelCentres()
  {
  ...
  void drawCapabilityMap(bot_lcmgl_t* lcmgl, const int orient = 0, const Eigen::Vector3d centre =
      Eigen::Vector3d(0, 0, 0), const bool draw_cubes = true);
  void drawActiveMap(bot_lcmgl_t* lcmgl, const int orient = 0, const Eigen::Vector3d centre =
      Eigen::Vector3d(0, 0, 0), const bool draw_cubes = true);
  void setEndeffectorPose(const Eigen::Matrix<double, 7, 1> pose);
  void drawOccupancyMap(bot_lcmgl_t* lcmgl, const unsigned int capability_map_voxel,
      const unsigned int orient, const Eigen::Vector3d centre = Eigen::Vector3d(0, 0, 0),
      const bool draw_cubes = true);
  void reduceActiveSet(const bool reset_active, const std::vector<Eigen::Vector3d> point_cloud,
      FPPOutput &output,
      const Eigen::Vector2d sagittal_range = Eigen::Vector2d(-M_PI / 3, M_PI / 3),
      const Eigen::Vector2d transverse_range = Eigen::Vector2d(-M_PI / 3, M_PI / 3),
      const Eigen::Vector2d height_range = Eigen::Vector2d(0.6, 1.1),
...
private:
  ...
  bool isActiveVoxel(const unsigned int voxel)
  { return active_voxels[voxel];}
  bool isActiveOrient(const unsigned int voxel, const unsigned int orient)
  { return active_orientations(voxel, orient);}
