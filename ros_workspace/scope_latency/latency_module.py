...
class Latency:
  ...
  def add_js(self, js_time, js_walltime):
    self.js_time.append(js_time)
    self.js_walltime.append(js_walltime)
  def add_jc(self, jc_time, jc_walltime):
    if (jc_time in self.js_time):
      ...
      if not self.latency_count % self.p:
        print "%d | %f newer msgs | %f sec delay [%d]" % (jc_time, (self.latency_step_cumsum/self.pf), self.latency_cumsum/self.pf, self.latency_count )
        self.latency_count =0
        self.latency_cumsum=0 
        self.latency_step_cumsum=0 
