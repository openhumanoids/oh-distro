// Minimal RGBD odometry using FoVis in ROS
//
// mfallon and hordurj
...
ros_fovis_rgbd::ros_fovis_rgbd() : it_(nh_), sync_(MySyncPolicy(10)),
      ...
      depth_producer_(kcal_), pose_(Eigen::Isometry3d::Identity()) // 3)
{
    ...
    if (use_sync_)
    {
        ...
        sync_.registerCallback( boost::bind(&ros_fovis_rgbd::imageCb, this, _1, _2, _3, _4) );
    } 

    odom_pub_ = nh_.advertise<nav_msgs::Odometry>("vo", 50);    
}

ros_fovis_rgbd::~ros_fovis_rgbd()
{
  delete depth_image_;
  delete [] depth_data_;
}

void ros_fovis_rgbd::imageCb(const sensor_msgs::ImageConstPtr& depth_image,
                             ...
                             const sensor_msgs::CameraInfoConstPtr& rgb_cam_info)
{
  ...
  try
  {
    //std::cout << depth_image->encoding << " is the encoding \n";
    if (depth_image->encoding == enc::TYPE_16UC1) {
      depth_ptr = cv_bridge::toCvShare(depth_image, enc::TYPE_16UC1);
    } else {
      std::cerr << "ros_fovis_rgbd::imageCb: Unknown encoding: " <<  depth_image->encoding << std::endl;
      exit(-1);
    }

    rgb_ptr = cv_bridge::toCvShare(rgb_image, enc::RGB8);
...
fovis::PrimeSenseCalibration* ros_fovis_rgbd::load_calibration()
{
...
{
    ros::init(argc, argv, "ros_fovis_rgbd");
