...
const string uri_base("https://bitbucket.org/osrf/sandia-hand/firmware/bin/");

int update_mobo()
{
  printf("updating mobo...\n");
  return 0;
}

int update_palm()
{
  printf("updating palm...\n");
  return 0;
}

int update_fmcb()
{
  printf("updating fmcb...\n");
  return 0;
}

#define BAIL_IF_FAIL(BOARD) \
  do { \
    int rv = update_##BOARD (); \
    if (rv) \
      return rv; \
  } while (0)
...
void usage()
{
  printf("usage: sandia_hand_firmware_updater SIDE CMD [OPTS]\n");
