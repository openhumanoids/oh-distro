...
void initializeEEPROM(void)
{
    ...
    Parameter[PARAMETER_THERMAL_R_WINDING_TO_STATOR] = 2.66; // Thermal restance winding to stator
    Parameter[PARAMETER_T_PLUS] = 90.0;
    Parameter[PARAMETER_T_MINUS] = 90.0;
    Parameter[PARAMETER_TAU_WINDING_TO_STATOR] = 1777.0;
    ...
    Parameter[PARAMETER_THERMAL_R_STATOR_TO_CASE] = 13.7; // Thermal restance stator to case
    Parameter[PARAMETER_TAU_STATOR_TO_CASE] = 22800.0;
...
static int handleCollectionCommand(uint8_t *commandPacket, uint8_t *outputBuffer)
{
    ...
    {
        controlSignal = round(motorCurrent * 1000.0); //convert to mA DEBUG!!!!!!!!!
        //controlSignal = round(statorTemperature * 1000.0); //convert to mA DEBUG!!!!!!!!!
    ...
    {
        controlSignal = round(housingTemperature * 100.0); //convert to hundredths of a degree
        outputBuffer[RESPONSE_PAYLOAD_OFFSET+responseSize] = controlSignal & 0x00FF;
        outputBuffer[RESPONSE_PAYLOAD_OFFSET+responseSize+1] = (controlSignal & 0xFF00) >> 8;
        responseSize += 2;
    }
    ...
    {
        controlSignal = round(windingTemperature * 100.0); //convert to hundredths of a degree
        outputBuffer[RESPONSE_PAYLOAD_OFFSET+responseSize] = controlSignal & 0x00FF;
        outputBuffer[RESPONSE_PAYLOAD_OFFSET+responseSize+1] = (controlSignal & 0xFF00) >> 8;
        responseSize += 2;
    }
...
static int handleMotorCommand(uint8_t *commandPacket, uint8_t *outputBuffer)
{
    ...
    uint16_t setpoint;
    thermal_state_t applySetpoint = THERMAL_DISCARD;
    int rejectSetpoint;
    ...
    {
        outputBuffer[RESPONSE_STATUSCODE_OFFSET] = OUT_OF_RANGE_ERROR;
    } 
    else
    {
        switch(applySetpoint)
        {
            ...
            case THERMAL_DISCARD:
                outputBuffer[RESPONSE_STATUSCODE_OFFSET] = MANDATORY_COOLDOWN_ERROR;
                break;
            case THERMAL_ERROR:
                outputBuffer[RESPONSE_STATUSCODE_OFFSET] = MISSING_THERMISTOR_ERROR;
                break;
            default:
                outputBuffer[RESPONSE_STATUSCODE_OFFSET] = UNKOWN_ERROR;
                break;
        }
    }
...
int main(void)
{
    ...
    readCurrentSignals(); // fill housing temp
    initializeThermalModel(); // must come after initializeParameters() and readCurrentSignals()
    ...
    while(1)
    {
        ...
        if(PID_runPID == 1)
        {
            ...
            readCurrentSignals();
            //LEDtoggle();
            updateThermalModel(housingTemperature, motorCurrent);
