...
class HandSensors
{
 public:
    ...
    void update(const HandSensors& other, const HandSensorsValid& valid)
    {
        ...
        for (int i=0; i<12; i++)
        {
            responses[i] = other.responses[i];
            //responseHistory[i] = other.responseHistory[i];
        }
        for (int i=0; i<5; i++)
            motorError[i] = other.motorError[i];
    };
    ...
    void print(const HandSensorsValid& valid) const
    {
        ...
        printf("  ");
        for (int i=0; i<12; i++)
            printf("%3d ", responseHistory[i]);
        printf("\n");

        printf("Motor Error:\n");
        for (int i=0; i<5; i++)
            printf("%d ", motorError[i]);
        printf("\n");
    ...
    int pack(unsigned char* buffer) const
    {
        ...
        for (int i=0; i<12; i++)
            ptr += ::pack(ptr, responses[i]);
        for (int i=0; i<12; i++)
            ptr += ::pack(ptr, responseHistory[i]);
        for (int i=0; i<5; i++)
            ptr += ::pack(ptr, motorError[i]);
    ...
    int unpack(const unsigned char* const buffer)
    {
        ...
        for (int i=0; i<12; i++)
            ptr += ::unpack(ptr, &responses[i]);
        for (int i=0; i<12; i++)
            ptr += ::unpack(ptr, &responseHistory[i]);
        for (int i=0; i<5; i++)
            ptr += ::unpack(ptr, &motorError[i]);
...
// convert the device order to finger number order
const uint8_t deviceMask_to_fingerNumber[12] = {0, 3, 4, 1, 2, 5, 6, 10, 8, 9, 7, 11};
