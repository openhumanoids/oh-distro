...
void Dexter::handle_cb(const HandPacket& msg)
{
    ...
    for (int i=0; i<12; i++)
        newmsg.responses[i] = msg.data.responses[i];

    for (int i=0; i<12; i++)
        newmsg.responseHistory[i] = msg.data.responseHistory[i];

    for (int i=0; i<5; i++)
        newmsg.motorError[i] = msg.data.motorError[i];
        ...
        if (msg->valid[i])
        {
            switch (msg->type[i])
            {
                ...
                case handle_msgs::HandleControl::VOLTAGE: 
                    ...
                    break;
                case handle_msgs::HandleControl::ANGLE:
                    cmd.motorCommand[i].type = FINGER_ANGLE;
                    break;
                default:
