...
HandleCommand annan_to_cmd(const AnnanBoard& data)
{
    ...
    else if (data.Button[3] && data.Button[2]) //position + velocity = finger angle control
    {
        if (lastState != 5)
        {
            for (int i=0; i<5; i++)
        ...
        for (int i=0; i<3; i++)
        {
            cmd.motorCommand[annanToFinger[i]].type = FINGER_ANGLE;
            cmd.motorCommand[annanToFinger[i]].valid = true;
            
            cmd.motorCommand[annanToFinger[i]].value = (data.Slider[i]-zeroPoint[i]) * (float)MAX_FINGER_ANGLE / (float)MAX_SLIDER;
        }
        // for (int i=3; i<5; i++)
        // {
