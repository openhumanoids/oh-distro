...
void readCurrentSignals()
{
    if(CURRENT_ADC.INTFLAGS & ADC_CH0IF_bm)
    {
        ...
        housingTemperature = Thermistor_RtoT(Thermistor_VtoR(thermistorVoltage));
        if(housingTemperature < 0) housingTemperature = 0;
        if(housingTemperature > 130) housingTemperature = 130;
    }
    ...
    if(CURRENT_ADC.INTFLAGS & ADC_CH1IF_bm)
    {
        ...
        float tempMotorCurrent = compensateCurrent(ADC_CODES_TO_AMPERES((float)rawMotorCurrent));
        ...
        // From external measurement
        if (motorCurrent > 0.117)
            motorCurrent -= 0.117;
